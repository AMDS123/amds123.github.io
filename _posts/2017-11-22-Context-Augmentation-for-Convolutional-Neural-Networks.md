...
深度卷积神经网络（ConvNets）最近被大量的标记数据所强化，已经为许多物体识别任务缩小了与人类表现的差距。这些令人印象深刻的结果引起了对ConvNets的理解和可视化的兴趣。在这项工作中，我们研究背景在图像分类任务中的作用。我们的研究结果表明，改变训练数据集的背景会对测试精度产生重大影响。此外，我们使用前景分割对象来增强现有的增强技术。这项工作的发现对于提高只有小数据集的可用性，创建数据集和创建合成图像的准确性非常重要。
