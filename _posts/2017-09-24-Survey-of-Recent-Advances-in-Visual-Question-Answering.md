...
视觉问答（VQA）提出了一个独特的挑战，因为它需要理解和编码多模态输入的能力 - 在图像处理和自然语言处理方面。该算法还需要学习如何对这种多模态表示进行推理，以便能够正确回答问题。本文提出了一个解决视觉问题回答问题的不同方法的调查。我们也在后面的部分描述了当前的艺术模型。本文特别介绍了各种算法提取图像特征，文本特征和预测答案的方法。我们还简要讨论了为评估VQA模型而进行的实验，并在不同的数据集上报告了他们的表现，包括新发布的VQA2.0 [8]。
