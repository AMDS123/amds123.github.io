---
layout: post
title: "Speeding up the K{ö}hler's method of contrast thresholding"
date: 2017-07-17 09:41:04
categories: arXiv_CV
tags: arXiv_CV
author: Guillaume Noyel (IPRI)
mathjax: true
---
...
K（o）hler方法是基于边界对比度的一种有用的多阈值技术，然而，直接算法具有太高的复杂度-O（N 2），即像素数为N的二次方，以足够的处理图像本文提出了一种加速K（o）hler方法的新算法，其复杂度为O（NM），M为灰度级数。所提出的算法被设计用于在当前的处理器中使用OpenMP（开放式多处理）和SIMD指令（单指令多数据）来进行并行化和向量处理。快速实现允许1800万像素图像中的增益因子为405，实时视频处理（增益因子为96）。

##### URL
[https://arxiv.org/abs/1707.05062](https://arxiv.org/abs/1707.05062)

##### PDF
[https://arxiv.org/pdf/1707.05062](https://arxiv.org/pdf/1707.05062)
