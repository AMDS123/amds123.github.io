...
在本文中，我们提出了一个有效的识别模型显着物体检测。我们的方法是基于散度背景和紧凑的前景线索的逐步机制。为了有效提高节点沿物体边界的区分度和物体区域之间的相似度，引入虚拟节点的概念构造图。为了去除不正确的输出，分别引入了用于选择背景种子的方案和用于生成紧凑性前景区域的方法。与以前的方法不同，我们根据相应节点与虚拟节点之间的关系计算每个节点的显着性值。为了实现显着的性能改进，我们提出了一个扩展的流形排列（EMR）算法，它巧妙地结合了抑制/主动节点和中间层次的信息。广泛的实验结果表明，所提出的算法在多个基准数据集上的不同评估度量方面对于最先进的显着性检测方法有利地进行。
