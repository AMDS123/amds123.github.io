...
我们提供IIT孟买英语 - 印地语平行语料库。该语料库是以前在公共领域提供的平行语料库的汇编，也是我们收集的新的平行语料库。该语料库包含149万个平行段，其中694个段在以前不在公有领域。语料库已经过机器翻译的预处理，并且我们在这个语料库上报告了基于短语的SMT和NMT翻译结果。这个语料库已经在亚洲语言转换研讨会（2016和2017）的两个版本的共享任务中使用。语料库可以免费用于非商业性研究。据我们所知，这是最大的公开可用的英语 - 印度语平行语料库。
