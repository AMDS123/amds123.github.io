...
本文提出了一个增加的MSCOCO数据集，语音添加到图像和文本。使用文本到语音（TTS）合成生成语音字幕，导致与图像配对的616,767个说明字幕（超过600小时）。为了听起来更加自然，信号中添加了不稳定性和速度扰动。每个语音信号（WAV）与一个JSON文件配对，该JSON文件包含对于说明标题中的每个词/音节/音素的确切时间码。这样的语料库可以用于语言和视觉（LaVi）任务，包括语音输入或输出而不是文本。通过对语料库的一个子集进行的初步研究（10h，10k的说明字幕），证明了这个语料库也可以研究用于无监督语音模式发现的多模式学习方案。
