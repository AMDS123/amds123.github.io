...
一个对象的多个实例经常出现在同一场景中，例如仓库中。无监督的多实例对象发现算法能够检测和识别这样的对象。我们使用这种算法为基于卷积神经网络（CNN）的分类器提供对象提议。与传统的区域提议算法相比，这导致评估的区域更少。另外，它使得能够使用对象的多个实例的联合概率，导致分类准确度的提高。所提出的技术也可以将单个类分成对应于不同对象类型的多个子类，从而实现分层分类。
