...
由于缺乏标注，未知野生人脸数据集中现代的偏见是什么样的。这样做的一个直接结果就是总的识别率只能对深度卷积神经网络（DCNNs）的泛化能力提供有限的见解。我们提出通过实证研究不同类型的数据集偏差对DCNN泛化能力的影响。使用综合生成的人脸图像，我们研究人脸识别率作为可解释的参数，如脸部姿势和光的函数。所提出的方法允许观察和比较不同DCNN体系结构的泛化性能的有价值的细节。在我们的实验中，我们发现：1）事实上，数据集偏差对DCNN的泛化性能有显着的影响。 2）DCNN可以很好地推广到看不见的照明条件和姿势变化中的大取样间隙。 3）我们揭示了当前DCNN体系结构的一个主要限制，这是当不同的身份不共享相同的姿态变化时难以概括的。 4）我们证明我们在综合数据上的发现也适用于从现实世界的数据中学习。我们的脸部图像生成器是公开可用的，使社区能够对其他DCNN体系结构进行基准测试。
