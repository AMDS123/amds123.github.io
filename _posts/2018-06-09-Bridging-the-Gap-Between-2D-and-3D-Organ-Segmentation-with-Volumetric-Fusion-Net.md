...
关于是否使用2D或3D深层神经网络进行体积器官分割一直存在争议。 2D和3D模型都有其优点和缺点。在本文中，我们提出了一个替代框架，它在不同的视点上对二维网络进行分割，并建立一个3D体积融合网络（VFN）来融合2D分割结果。 VFN相对较浅并且包含的​​参数比大多数3D网络少得多，这使得我们的框架在集成3D信息进行分割时更加高效。我们单独训练和测试分割和融合模块，并提出一种名为交叉增强的新策略，以充分利用有限的训练数据。我们评估我们的几个具有挑战性的腹部器官的框架，并验证其在现有的二维和三维方法分割准确性和稳定性方面的优势。

##### URL
[http://arxiv.org/abs/1804.00392](http://arxiv.org/abs/1804.00392)

##### PDF
[http://arxiv.org/pdf/1804.00392](http://arxiv.org/pdf/1804.00392)
