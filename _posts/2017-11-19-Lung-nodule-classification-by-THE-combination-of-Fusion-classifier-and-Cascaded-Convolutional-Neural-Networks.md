...
肺结节分类是一类不平衡的问题，因为发现结节的频率远低于非结节。在阶级失衡的问题上，传统的分类者倾向于被大多数阶级所淹没，而忽视了少数阶级。我们发现，级联卷积神经网络可以对我们以前的研究中的类不平衡结节候选数据集精确地分类结节候选者。在本文中，我们提出融合分类器结合级联卷积神经网络模型。为了融合模型，首先使用卷积神经网络模型计算结节概率。然后，融合分类器通过结节概率进行训练和测试。在自由接受者操作特征（FROC）曲线分析中，所提出的方法在每次扫描的4个和8个假阳性分别获得94.4％和95.9％的灵敏度。
