...
给定图像和口头语音标题的集合，我们提出一种方法来发现连续语音信号中的类似单词的声学单元，并将它们接到语义相关的图像区域。例如，我们的模型能够在话语中检测“灯塔”这个单词的口头实例，并将它们与包含灯塔的图像区域联系起来。我们不使用任何形式的传统自动语音识别，也不使用任何文本转录或传统的语言注释。我们的模型有效地实现了一种口语获取的形式，其中计算机学习不仅通过声音识别词类，而且通过将其语义学放在图像中来丰富它学习的词语。
