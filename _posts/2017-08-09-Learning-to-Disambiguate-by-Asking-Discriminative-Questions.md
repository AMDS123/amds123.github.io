...
提问的能力是收集信息以便了解世界并解决歧义的有力工具。在本文中，我们探讨了一个产生歧视问题的新颖问题，以帮助消除视觉实例的歧义。我们的工作可以看作是丰富的图像字幕和问题答案研究的补充和新的延伸。我们引入了第一个大规模的数据集，超过10,000个仔细注释的图像 - 问题元组，以便于基准测试。特别是，每个元组平均包含一对图像和4.6个有区别的问题（作为阳性样本）和5.9个无差别的问题（作为阴性样本）。此外，我们提出了一个有效的视觉歧视问题生成方法。该方法可以以弱监督的方式进行训练，而不具有区分性图像 - 问题元组，而只是存在的视觉问题回答数据集。通过定量评估和用户研究，对有代表性的基线显示有希望的结果。
