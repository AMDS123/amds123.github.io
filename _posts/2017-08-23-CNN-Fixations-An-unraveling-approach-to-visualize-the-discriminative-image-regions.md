...
categories: arXiv_CV
tags: arXiv_CV Caption CNN Classification Prediction Detection Recognition
...
深度卷积神经网络（CNN）已经彻底改变了各个领域的视觉研究领域，并且在分类，检测，字幕等多项任务中被采用了前所未有的方法。然而，它们在内部工作中几乎没有透明度，常被当作黑盒子来处理提供出色的表现。在这项工作中，我们的目标是通过提供网络预测的视觉解释来缓解CNN的这种不透明性。我们的方法可以分析各种基于CNN的视觉应用培训模型，如对象识别和字幕生成。与现有的方法不同，我们通过解开正向传递操作来实现这一点。提出的方法利用了层次层次结构中的特征依赖关系，揭示了指导网络预测的区分性图像位置。我们将这些位置命名为CNN-Fixations，大致类似于人眼的注视。我们的方法是一个通用的方法，不需要架构修改，额外的培训或梯度计算，并计算重要的图像位置（CNN固定）。我们通过各种应用演示，我们的方法能够在不同的网络架构，不同的视觉任务和数据模式上对差异化图像位置进行本地化。
