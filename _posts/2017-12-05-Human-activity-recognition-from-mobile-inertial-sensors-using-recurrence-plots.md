...
惯性传感器目前在大多数移动设备中都存在，并且这些设备在大多数日常活动中被人们使用。在本文中，我们提出了一种基于惯性传感器的人类活动识别方法，采用递归图（RP）和视觉描述符。所提出的方法的流水线如下：从传感器数据计算RP，计算来自RP的视觉特征，并将其用在机器学习协议中。由于RP生成纹理视觉模式，我们将传感器数据分类问题转化为纹理分类问题。基于加速度计数据对人类活动进行分类的实验表明，所提出的方法获得最高的精度，优于传感器数据直接提取的时域和频域特征。使用RGB RP时获得最佳结果，其中每个RGB通道对应于独立加速度计轴的RP。
