...
地球观测卫星长期以来一直在监测地球表面，卫星拍摄的图像中含有大量有价值的数据。然而，手动分析如此庞大的数据是非常困难的工作。因此，卫星图像需要一种自动物体检测的方法来促进有效的数据分析。本文描述了从高阶局部自相关到多光谱卫星图像目标检测的一种新的图像特征。该功能已扩展到除空间关系之外的频谱相互关系以充分利用多光谱信息。为了评估所提出的特征扩展的有效性而进行的对象检测任务的实验结果表明，与现有方法相比，该特征实现了更高的性能。
