...
尽管现有的光学标记读取（OMR）系统和设备具有很高的准确性，但仍然存在一些限制。在这项工作中，我们的目标是在测试中减少多项选择题（MCQ）的限制。我们使用图像配准技术从答卷中提取答案框。与依赖于简单图像处理步骤来识别提取的答案框的其他系统不同，我们通过训练分类器来识别每个答案框的类别（即，确认，划掉和空白的答案），从另一个角度解决问题。这使我们能够处理各种阴影和标记模式，并区分选择和取消的答案。所有现有的机器学习技术都需要大量的例子来训练分类器，因此我们提出一个由五个实际的MCQ测试和一个有不同答题纸模板的测验组成的数据集。我们评估两种分类策略：直接方法和两阶段分类器方法。我们测试两个手工特征方法和一个卷积神经网络。最后，我们提出了一个易于使用的图形用户界面的拟议系统。与现有的OMR系统相比，所提出的系统被认为具有更高的精度和最小的约束。我们相信所提出的工作将进一步指导OMR系统的发展，而不受限于无模板MCQ测试。
