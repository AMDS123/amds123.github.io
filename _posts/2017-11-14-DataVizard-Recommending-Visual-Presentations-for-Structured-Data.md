...
选择合适的数据视觉呈现方式，使其保留底层数据的语义，同时提供数据的直观总结，这是数据分析的重要步骤，通常是最后一步。不幸的是，这也是一个重要的人类努力的一个步骤，从分析阶段结构化结果中的列组选择开始，到通过试验各种替代方案来选择正确的可视化。在本文中，我们描述了我们的\ DataVizard系统，旨在通过自动为结构化结果推荐最合适的视觉表示来减少这种开销。具体而言，我们考虑以下两种情况：首先，当需要可视化结构化查询（如SQL）的结果时;而第二种方法是获得具有相关简短描述的数据表（例如，来自Web的表格）。使用真实世界数据库查询（及其结果）的语料库和从Web爬取的许多统计表，我们显示DataVizard能够以高准确度推荐可视化演示。我们还介绍了我们进行的用户调查的结果，以评估用户对所显示的图表与数据的明文标题的适用性的看法。
