...
目标检测是计算机视觉领域的一个主要问题，并得到了广泛的研究。现有的大部分检测工作都依赖于劳动密集型的监督，比如对象的地面边界框或者至少是图像级的注释。相反，我们提出了一种目标检测方法，它不需要对目标任务进行任何形式的监督，通过利用免费提供的网络图像。为了促进从Web图像有效的知识转移，我们引入了两个关键创新的多实例多标签领域自适应学习框架。首先，我们提出了一个关注前景对象的实例级对抗域自适应网络，将对象出现从网页域转移到目标域。其次，为了保留传递对象特征的类特定语义结构，我们提出了通过伪强标签生成来跨领域传递监督的同时传输机制。通过我们的端到端框架，同时学习弱监督的检测器和跨领域的知识转移，我们在基准数据集的基准方法上取得了显着的进步。
