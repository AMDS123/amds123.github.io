...
我们提出了一种从多语种和多模态数据中学习嵌入的新型判别模型，这意味着我们的模型可以利用多种语言的图像和描述来提高嵌入质量。为此，我们引入了修改的成对比估计优化函数作为我们的训练目标。我们评估我们在图像 - 语句排序（ISR），语义文本相似性（STS）和神经机器翻译（NMT）任务中的嵌入。我们发现，额外的多语言信号导致了ISR和STS任务的改进，而歧视成本也可以用于NMT模型生成的$ n $ -best列表的重新排序，产生了很大的改进。
