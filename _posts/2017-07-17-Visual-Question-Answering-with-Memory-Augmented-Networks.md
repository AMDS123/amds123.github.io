...
本文利用记忆增强的神经网络来预测对视觉问题的准确答案，即使这些答案在训练集中很少发生。内存网络包含内部和外部存储器块，并有选择地关注每个培训示例。我们证明，记忆增强的神经网络能够保持对稀缺训练样本的相对长期的记忆，这对于视觉问题答案是重要的，因为在一般VQA环境中的重尾分布的答案。在两个大型基准数据集上的实验结果显示了所提出的算法与现有技术的比较的良好性能。
