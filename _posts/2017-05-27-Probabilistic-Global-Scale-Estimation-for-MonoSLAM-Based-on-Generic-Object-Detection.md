...
本文提出了一种基于卡尔曼滤波的单目SLAM算法估计三维重建模型全局尺度的新方法。基于快速通用对象检测的最新进展，我们的贝叶斯框架将属于一组广泛的预定义类的检测对象的高度先验集成在一起。每个观察都是在单帧上产生的，所以我们不需要沿着视频帧进行数据关联处理。这是因为我们将高度先验与地图特征投影落在对象检测区域内的图像地点处的图像区域大小相关联。我们目前非常有前途的结果这个方法获得了不同的对象类的几个实验。
