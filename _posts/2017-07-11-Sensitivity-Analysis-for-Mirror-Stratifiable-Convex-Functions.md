...
本文为具有强几何结构的一类凸函数提供了一套灵敏度分析和活动识别结果，我们创造了“镜面可分级”。这些功能是这样的，即在空间的原始和双重分层之间存在双向映射，称为分层。这种配对对跟踪可以通过参数化优化问题的解决方案或通过迭代优化算法可识别的层是至关重要的。这类功能包括在信号和图像处理，机器学习和统计中常规使用的所有调整器。我们证明这个“镜像 - 分层”结构具有良好的灵敏度理论，使我们能够研究小扰动下优化问题解的稳定性，以及一阶近邻分裂型算法的活动识别。文献中的现有结果通常假定，在非退化条件下，与最小值相关的有效集对于小扰动是稳定的，并且在有限时间内通过优化方案来识别。相反，我们的结果不需要任何非退化假设：因此，最优有效集合不一定是稳定的，但是我们能够精确地追踪可识别层的集合。我们证明这些结果在求解通过正规化来挑战不适定的逆向问题，这是非退化条件未被满足的典型情况。我们的理论结果，通过数值模拟进行说明，可以通过定位可能由这些解决方案潜在识别的所有低维地层的集合来表征正则化解的不稳定行为。

##### URL
[https://arxiv.org/abs/1707.03194](https://arxiv.org/abs/1707.03194)

##### PDF
[https://arxiv.org/pdf/1707.03194](https://arxiv.org/pdf/1707.03194)
