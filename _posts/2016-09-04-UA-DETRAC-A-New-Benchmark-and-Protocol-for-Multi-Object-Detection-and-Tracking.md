...
近年来，由于其广泛的应用，开发了许多有效的多目标跟踪（MOT）方法。 MOT方法的现有性能评估通常通过使用相同的固定对象检测结果进行比较来将对象跟踪步骤与对象检测步骤分开。在这项工作中，我们使用Albany DETection和tRACking（UA-DETRAC）基准数据集，使用新的大规模大学，对目标检测精度对整体MOT性能的影响进行了全面的定量研究。 UA-DETRAC基准数据集由100个具有挑战性的视频序列组成，这些视频序列从真实世界的交通场景（超过140,000帧，丰富的注释，包括遮挡，天气，车辆类别，截断以及车辆边界框）中捕获，用于物体检测，目标跟踪和MOT系统。我们评估了由最先进的物体检测和物体追踪方法组合而成的完整MOT系统。我们的分析表明了目标检测精度对MOT系统性能的复杂影响。基于这些观察，我们为MOT系统提出了新的评估工具和指标，考虑了对象检测和对象跟踪以进行全面的分析。
