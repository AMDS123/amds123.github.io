...
我们目前在包括自动人脸分析在内的许多计算机视觉应用中所见到的进展将不可能在没有收集和注释大型视觉数据库的巨大努力的情况下实现。为此，我们提出了一个新的动态高分辨率3D面（超过180万个三维网格）的大型数据库4DFAB。 4DFAB包含了四个不同的会议跨越五年时间捕获的180个科目的录音。它包含显示自发和姿势面部行为的主题的4D视频。该数据库可用于脸部和面部表情识别，以及行为生物识别。它也可以用来学习非常强大的Blendshapes参数化面部表情。在本文中，我们进行了几个实验，并展示了数据库在各种应用中的用处。该数据库将公开提供用于研究目的。
