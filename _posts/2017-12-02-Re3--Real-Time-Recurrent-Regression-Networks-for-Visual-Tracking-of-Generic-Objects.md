...
强大的对象跟踪需要对被跟踪对象的了解和理解：它的外观，运动以及它随着时间的变化。跟踪者必须能够修改其基本模型并适应新的观察。我们提出Re3，这是一个实时的深度对象跟踪器，能够将时间信息整合到模型中。我们不是将注意力集中在有限的对象上，或是在测试阶段对模型进行训练来追踪特定的实例，而是将我们的通用跟踪器预加载到各种各样的对象上，并在飞行中进行有效更新。 Re3同时跟踪并更新外观模型，只需一个正向传递。这个轻量级模型能够以150 FPS的速度跟踪物体，同时在具有挑战性的基准上获得有竞争力的结果。我们还表明，我们的方法处理临时遮挡比其他可比较的跟踪器更好地使用实验，直接测量闭塞序列的性能。
