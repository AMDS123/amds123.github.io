...
利用CNN的最新进展，从单一图像预测深度的能力对视觉社区的兴趣越来越大。无监督的学习策略是特别有吸引力的，因为他们可以在学习过程中使用更大和更多的单目视频数据集，而不需要地面真实深度或立体声。在以前的作品中，必须确定独立的姿势和深度的CNN预测因子，以使它们的联合输出使光度误差最小化。受直接视觉测距法（DVO）的最新进展的启发，我们认为，可以在没有姿势CNN预测器的情况下学习CNN深度预测器。此外，我们凭经验证明，DVO的差异化实施以及新颖的深度标准化策略 - 显着提高了使用单目视频进行训练的最新技术的性能。
