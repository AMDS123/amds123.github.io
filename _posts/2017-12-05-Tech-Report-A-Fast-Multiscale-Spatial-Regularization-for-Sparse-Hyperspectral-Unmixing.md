...
大光谱库中的稀疏高光谱分解被认为是在许多应用中避免端元提取算法的局限性。这种策略往往导致不适定的逆问题，这可以从空间正则化策略中受益。虽然现有的空间正则化方法改善了问题调节并促进了分段光滑的解决方案，但却导致了大的非光滑优化问题。因此，在解混问题中有效引入空间环境仍然是一个挑战，也是许多现实世界应用的必要条件。本文提出了一种新的稀疏分解的多尺度空间正则化方法。该方法使用基于超像素的信号自适应空间多尺度分解将解混问题分解为两个较简单的问题，一个在近似域而另一个在原始域。使用合成数据和真实数据的仿真结果表明，所提出的方法可以胜过基于最先进的基于全变差的算法，其计算时间可以与未经规律化的算法相当。

##### URL
[https://arxiv.org/abs/1712.01770](https://arxiv.org/abs/1712.01770)

##### PDF
[https://arxiv.org/pdf/1712.01770](https://arxiv.org/pdf/1712.01770)
