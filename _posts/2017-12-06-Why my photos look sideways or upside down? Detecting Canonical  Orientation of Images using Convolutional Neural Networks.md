...
图像方向检测需要高级别的场景理解。人类使用物体识别和情景场景信息来正确定位图像。在文献中，图像方向检测的问题大多是使用低层次的视觉特征，而一些方法则很少包含易于检测的语义线索以获得较小的改进。图像中的大量语义内容使定向检测具有挑战性，因此现有方法与人类行为之间存在较大的语义差距。另外，文献中现有的方法报告的检测率差异很大，主要是由于数据集的差异较大以及用于评估的测试图像的种类有限。在这项工作中，我们第一次利用了深度学习的力量，并使用最大的训练数据集对预先训练的卷积神经网络进行调整，以适应图像定向检测任务。在不同的公共数据集上对我们的模型进行了广泛的评估，结果表明它能够正确定位一大组不受限制的图像。它也明显优于最先进的技术，达到与人类非常接近的准确度。

##### URL
[https://arxiv.org/abs/1712.01195](https://arxiv.org/abs/1712.01195)
