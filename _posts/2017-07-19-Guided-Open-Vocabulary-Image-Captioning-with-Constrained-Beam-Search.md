...
现有的图像字幕模型不能很好地推广到包含新颖场景或对象的域外图像。这种限制严重阻碍了这些模型在现实世界中处理图像的应用程序的使用。我们用一种灵活的方法解决了这个问题，使现有的深层字幕架构能够在测试时利用图像标记器，而无需重新培训。我们的方法使用受限束搜索来强制在输出中包含选定的标签词，以及固定的，预训练词嵌入，以促进词汇扩展到以前看不见的标签词。使用这种方法，我们实现了对MSCOCO的域外字幕（以及对于域内字幕的改进结果）的最先进的结果。也许令人惊讶的是，我们的结果明显优于将相同的标签预测结合到学习算法中的方法。我们还表明，通过利用地面实况标签，我们可以显着提高生成的ImageNet字幕的质量。
