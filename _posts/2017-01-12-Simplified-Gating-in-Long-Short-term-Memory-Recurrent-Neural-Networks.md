...
标准的LSTM递归神经网络虽然在远程依赖序列应用中非常强大，但具有高度复杂的结构和相对较大的（自适应）参数。在这项工作中，我们提出标准的LSTM递归神经网络结构和通过消除输入信号，偏差和来自单个门控信号的隐藏单元信号的组合而获得的三个新的参数减少的变体之间的经验比较。在两个序列数据集上的实验表明，三种新的变体（简称为LSTM1，LSTM2和LSTM3）可以实现与具有较少（自适应）参数的标准LSTM模型相当的性能。
