---
layout: post
title: "Driver Action Prediction Using Deep Recurrent Neural Network"
date: 2017-06-07 17:00:08
categories: arXiv_CV
tags: arXiv_CV Knowledge RNN Prediction Detection Relation
...
高级驾驶员辅助系统（ADAS）可以通过有效的驾驶员行为预测（DAP）得到显着改善。及时准确地预测驾驶员行为可以帮助减轻潜在的不安全驾驶行为的影响，并避免可能发生的事故。本文将驾驶员行为预测作为时间序列异常预测问题。尽管在这种情况下异常（驾驶员感兴趣的动作）检测可能是微不足道的，但是寻找始终在异常之前的模式需要在多模态感觉输入中搜索或提取特征。我们提出了这样的驾驶员行为预测系统，包括实时数据采集，处理和学习框架，用于预测未来或即将发生的驾驶员行为。除了传统的车辆动力学之外，所提出的系统还包括基于相机的驾驶环境和驾驶员本身的知识。然后使用深度双向递归神经网络（DBRNN）来学习感官输入与即将发生的驾驶员行为之间的相关性，从而实现准确的高水平行动预测。所提出的系统比其他现有系统在驾驶员行为预测任务方面执行得更好，并且可以在驾驶员执行动作之前5秒的持续时间内准确预测关键驾驶员动作，包括加速，制动，车道变换和转弯。

##### URL
[https://arxiv.org/abs/1706.02257](https://arxiv.org/abs/1706.02257)

##### PDF
[https://arxiv.org/pdf/1706.02257](https://arxiv.org/pdf/1706.02257)
