...
医院中储存的信息的数字化现在允许以文本格式开发医疗数据，如电子健康记录（EHR），最初是为了流行病学之外的其他目的而收集的。手动搜索和分析这些数据的操作变得单调乏味。近年来，强调使用自然语言处理（NLP）工具来自动提取EHR中包含的信息，对其进行结构化并对该结构化信息进行统计分析。现有方法的主要困难是同义词或本体词典的需求，这些词典大部分仅以英文提供，不包括本地或自定义符号。在这项工作中，由肿瘤学家组成的团队成为领域专家和数据科学家，开发了一个自定义的基于NLP的系统来处理和构建乳腺癌患者的文本临床报告。该工具依赖于标准文本挖掘技术和先进的同义词检测方法的结合。它允许通过检索诸如病史，肿瘤特征，治疗反应，复发和预后等指标进行全球分析。该方法的多功能性可以容易地获得新的指标，从而为回顾性研究打开了一个大幅度减少手工工作量的方式。由于不需要生物医学注释器或预先定义的本体，根据与手动结构化文件的比较，这种与语言无关的方法对几个感兴趣的概念提取了很好的提取精度，而不需要任何现有的具有本地或新的符号的语料库。

##### URL
[http://arxiv.org/abs/1712.02259](http://arxiv.org/abs/1712.02259)

##### PDF
[http://arxiv.org/pdf/1712.02259](http://arxiv.org/pdf/1712.02259)
