...
图像合成是一种通过将内容从一个图像插入到另一个图像来生成逼真而虚假的图像的方法。先前在合成中的工作集中于改善用户选择的前景片段和背景图像的外观兼容性（即颜色和照明一致性）。在这项工作中，我们改为开发一个完全自动化的合成模型，另外学习从只有输入图像背景的大集合中选择和转换兼容的前景部分。为了简化任务，我们通过关注人类实例组成来限制我们的问题，因为人类细分与他们的背景显示出强烈的相关性，并且由于大量注释数据的可用性。我们开发了一种新颖的分支卷积神经网络（CNN），联合预测给定背景图像的候选人位置。然后，我们使用预先训练的深度特征表示从大型分段数据库检索人员实例。实验结果表明，我们的模型可以生成视觉上令人信服的合成图像。我们还开发了一个用户界面来展示我们方法的潜在应用。
