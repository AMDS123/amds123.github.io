...
受到孩子学习经验的启发 - 首先被教导，然后被观察和质疑，我们调查了一个批判性监督的学习方法，用于物体检测。具体而言，我们提出了一个包含负面对象建议，关键示例挖掘和机器引导问答（QA）标签等几个新颖组件的教学观察问题（TOA）方法。为了考虑对时间和性能进行联合标注，我们开发了新的评估方法来比较TOA方法的性能和完全和弱监督的学习方法。在PASCAL VOC和Caltech基准数据集上进行了大量的实验。 TOA方法显着改善了监督力度不够的表现，但只需要全面监督的标记时间的3-6％。还分析了每个新颖组件的有效性。
