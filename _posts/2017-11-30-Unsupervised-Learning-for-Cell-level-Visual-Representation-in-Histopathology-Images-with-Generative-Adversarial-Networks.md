...
categories: arXiv_CV
tags: arXiv_CV Adversarial Segmentation GAN Image_Classification Represenation_Learning Classification
...
细胞的视觉属性，如核形态和染色质的开放性，对组织病理学图像分析至关重要。通过学习细胞级别的视觉表示，我们可以获得丰富的特征组合，这些特征可以高度重用于各种任务，如细胞级分类，细胞核分割和细胞计数。在本文中，我们提出了一个统一的生成对抗网络架构与损失的一个新的表达式在无监督设置执行稳健的细胞水平的视觉表示学习。我们的模型不仅没有标签，而且易于训练，而且能够进行细胞级别的无监督分类和可解释的可视化，这在无监督的骨髓细胞成分分类中取得了有希望的结果。基于提出的细胞水平视觉表征学习，我们进一步开发了利用细胞成分进行组织病理学图像分类的流水线，其优点在骨髓数据集上得到了展现。
