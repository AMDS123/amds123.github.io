...
随着机器学习方法进入涉及最终用户的实际应用程序，人与智能机器之间的交互和协作变得越来越重要。虽然许多先前的工作在于自然语言和视觉的交叉，例如图像字幕或从文本描述生成图像，但是较少关注于使用语言来指导或改善学习的视觉处理算法的性能。在本文中，我们探索了通过用户输入灵活地引导训练的卷积神经网络以在推理期间改善其性能的方法。我们通过在网络中插入一个充当空间语义指南的层来实现。本指南经过培训，可以直接通过能量最小化方案修改网络的激活，也可以通过将人类语言查询转换为交互权重的循环模型间接修改。学习口头交互是完全自动的，不需要手动文本注释。我们在两个数据集上评估该方法，表明指导预先训练的网络可以提高性能，并提供对指南和CNN之间的交互的广泛见解。
