...
在语言习得过程中，婴儿可以通过视觉线索来使用口语。机器人同样可以使用音频和视觉传感器。最近的工作表明，图像和口头字幕可以映射到一个有意义的公共空间，允许使用语音检索图像，反之亦然。在这个与未转录字幕配对的图像设置中，我们考虑是否可以使用计算机视觉系统来获取语音的文本标签。具体而言，我们使用图像到词语的多标签视觉分类器来标记具有软文本标签的图像，然后训练一个神经网络从语音映射到这些软目标。我们表明，由此产生的语音系统能够预测哪些词出现在话语中---充当口语袋分类器---没有看到任何平行的言语和文本。我们发现该模型经常混淆语义相关的词，例如“人”和“人”，使得它作为一个语义关键词检测器更加有效。
