...
传统的图像识别涉及使用单个对象焦点（ILSVRC，AlexNet和VGG）识别纵向类型图像中的关键对象。最近的方法考虑了密集图像识别 - 使用适当的边界框分割图像并在这些边界框内执行图像识别（语义分割）。 Visual Genome数据集[5]试图将这些不同的方法连接到每个子任务的内聚数据集 - 边界框生成，图像识别，字幕和新操作：场景图生成。我们的重点是使用此类场景图对图像数据库执行图搜索，以根据搜索条件全面检索图像。我们开发了一种方法，用于在图数据库中存储场景图和元数据（使用Neo4J），并基于图搜索查询执行图像的快速近似检索。我们处理比单个对象搜索更复杂的查询，例如“女孩吃蛋糕”检索包含指定关系和变体的图像。
