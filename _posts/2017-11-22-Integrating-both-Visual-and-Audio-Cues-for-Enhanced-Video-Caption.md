...
视频标题是指自动为特定的短视频剪辑生成描述性语句，近来取得了显着的成效。然而，大多数现有的方法更注重视觉信息，而忽略了同步音频线索。我们提出了三种多模式深度融合策略，以最大化视觉 - 音频共振信息的好处。第一个探讨了从低到高顺序融合的交叉模式特征的影响。第二种方式是通过共享相应前端网络的权重来建立视音频的短期依赖关系。第三，通过跨视觉和音频模式共享多模态记忆，将时间依赖性延伸到长期。大量的实验验证了我们的三种跨模式融合策略在两个基准数据集（包括Microsoft Research Video to Text（MSRVTT）和Microsoft Video Description（MSVD））上的有效性。值得一提的是，分享权重可以有效地协调视音频特征融合，并达到BELU和METEOR指标的最新性能表现。此外，我们首先提出了一个动态多模态特征融合框架来处理部分模态缺失的情况。实验结果表明，即使在音频不存在模式下，借助附加的音频模态推断模块仍然可以获得可比较的结果。
