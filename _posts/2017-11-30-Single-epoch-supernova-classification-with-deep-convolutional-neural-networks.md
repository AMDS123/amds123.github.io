...
超新星Ia型（SNeIa）在探索宇宙膨胀的历史中起着重要的作用，因为它们是我们可以准确测量到物体距离的最着名的标准蜡烛。找到大样本的SNeIa并调查其详细特征已经成为宇宙学和天文学的一个重要问题。现有的方法依赖于一种光度测量方法，首先精确测量超新星候选的亮度，然后将结果拟合成亮度时间变化的参数函数。然而，它不可避免地需要多时期的观测和复杂的亮度测量。在这项工作中，我们提出了一种新的分类SNeIa的方法，通过将最先进的计算机视觉方法有效地整合到标准的光度测量方法中，而无需任何复杂的测量，从单历元观测图像简单地分类。我们的方法首先建立一个卷积神经网络，用于估计望远镜图像中超新星的亮度，然后构建另一个分类神经网络，其中估计的亮度和观测日期被用作分类的特征。两个神经网络都集成在一个单一的深度神经网络中，从观察图像直接对SNeIa进行分类。实验结果表明了该方法的有效性，揭示了分类性能可与现有的具有多时期观测的光度学方法相媲美。
