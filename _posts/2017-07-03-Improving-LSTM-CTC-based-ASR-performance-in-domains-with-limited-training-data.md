...
本文研究了基于长时短期记忆（LSTM）神经网络的自动语音识别（ASR）系统与连接主义时态分类（CTC）损失函数训练和基于混合深度神经网络（DNN）数据有限的域上的交叉熵（CE）损失函数。我们逐步进行了一系列实验，对基于EESEN工具包的LSTM-CTC ASR系统在Librispeech 100hr（train-clean-100）语料库上进行培训的基线进行了逐步改进。我们的结果显示，通过数据增强和正则化的有效结合，基于LSTM-CTC的系统可以超过基于相同数据训练的基于强度Kaldi的基线的性能。

##### URL
[https://arxiv.org/abs/1707.00722](https://arxiv.org/abs/1707.00722)

##### PDF
[https://arxiv.org/pdf/1707.00722](https://arxiv.org/pdf/1707.00722)
