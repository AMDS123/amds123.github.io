...
在遥感图像的解译中，由不同的传感器提供的一些图像可能是可以理解的。为了更好地对这些图像进行视觉感知，必须进行一系列预处理和基本校正，然后运行一系列主要处理步骤，以更精确地分析图像。有几种处理方法依赖于遥感图像的类型。本文讨论的方法，即图像融合，是使用光学图像的自然色彩为灰度卫星图像添加色彩，这使我们能够更好地观察Landsat-8的OLI传感器的HR图像。之前已经进行了重点在于融合技术细节的这个过程;但是，我们将要应用插值过程的概念。实际上，我们看到许多重要的软件工具，如ENVI和ERDAS，因为最着名的遥感图像处理工具只有经典插值技术（如双线性（BL）和双立方/立方卷积（CC））。因此，基于ENVI和ERDAS的图像融合领域的研究，甚至其他融合研究往往不使用新的更好的插值器，主要集中在融合算法的细节上，以获得更好的质量，所以我们只关注插值对融合的影响质量在Landsat-8多光谱图像。这种方法的重要特点是在实践中使用统计的，自适应的和边缘引导的插值方法来改善图像中的颜色质量。数值模拟显示，在基于MRF的图像中选择合适的插值技术比传统的插值器产生更好的质量。
