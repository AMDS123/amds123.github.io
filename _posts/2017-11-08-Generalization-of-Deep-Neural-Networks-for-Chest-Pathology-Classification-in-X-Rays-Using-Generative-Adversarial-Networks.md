...
categories: arXiv_CV
tags: arXiv_CV Adversarial Sparse GAN CNN Classification
...
医疗数据集往往是高度不平衡的，超过代表常见的医疗问题，稀少代表罕见的问题。我们建议模拟图像中的病理学，以克服上述限制。使用胸部X射线作为模型医学图像，我们实施一个生成对抗网络（GAN）来创建基于适度大小的标记数据集的人造图像。我们采用真实图像和人造图像的组合来训练深度卷积神经网络（DCNN）以检测五类疾病的病理学。我们进一步证明，使用GAN生成的图像增强原始的不平衡数据集，与使用原始数据集单独训练的相同DCNN相比，使用所提出的DCNN提高了胸部病理学分类的性能。这种改进的性能主要归因于使用GAN生成的图像的数据集的平衡，其中在示例图像中缺少的图像类别被优先增强。
