...
在本文中，我们处理当数据在离散域上时推断因果方向的问题。通过考虑原因分布$ P（X）$和条件分布映射导致$ P（Y | X）$作为独立随机变量，我们建议通过比较$ P之间的距离相关性来推断因果方向（ X）$和$ P（Y | X）$与$ P（Y）$和$ P（X | Y）$之间的距离相关性。如果$ P（X）$和$ P（Y | X）$之间的依赖系数较小，我们推断“$ X $导致$ Y $”。进行实验以显示所提出方法的性能。

##### URL
[http://arxiv.org/abs/1803.07712](http://arxiv.org/abs/1803.07712)

##### PDF
[http://arxiv.org/pdf/1803.07712](http://arxiv.org/pdf/1803.07712)
