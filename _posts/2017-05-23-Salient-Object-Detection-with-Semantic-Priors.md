...
显着物体检测已成为计算机视觉和人工智能研究等认知与计算科学中的热门话题。在本文中，我们建议将\ textit {语义先验}集成到显着的对象检测过程中。我们的算法由三个基本步骤组成。首先，基于从数据中学到的显式显着性先验得到的语义分割，得到显式的显着图。接下来，基于训练模型计算隐式显着性图，该模型将嵌入区域特征中的隐式显着性先验与显着性值进行映射。最后，将显式语义映射和隐式映射自适应地融合，形成均匀覆盖感兴趣对象的像素精确的显着图。我们进一步评估了两个具有挑战性的数据集，即ECSSD和HKUIS的建议框架。广泛的实验结果表明，我们的方法胜过其他最先进的方法。
