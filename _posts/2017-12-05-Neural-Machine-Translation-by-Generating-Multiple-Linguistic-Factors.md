...
Factored神经机器翻译（FNMT）建立在使用神经网络输出侧的词（因子）的形态和语法分解的思想基础上。该架构解决了MT中出现的两个众所周知的问题，即目标语言词汇的大小和翻译中产生的未知令牌的数量。 FNMT系统旨在管理更大的词汇量并缩短训练时间（针对具有相同目标语言词汇量的系统）。而且，我们可以产生不属于词汇表的语法正确的单词。 FNMT模型根据IWSLT'15英语到法语的任务进行评估，并与基线词和基于BPE的NMT系统进行比较。报告中有希望的定性和定量结果（就BLEU和METEOR而言）。
