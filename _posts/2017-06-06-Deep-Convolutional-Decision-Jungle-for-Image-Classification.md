...
我们提出了一种称为深度卷积决策丛林（CDJ）的新方法及其图像分类学习算法。 CDJ保持标准卷积神经网络（CNN）的结构，即多层多重响应图完全连接。卷积层和完全连接层中的每个响应图或节点有选择地响应类标签s.t.每个数据采样通过这些激活节点的特定软路由传播。所提出的方法CDJ自动学习特征，而决策森林和丛林需要预先定义的特征集。与CNN相比，该方法嵌入了使用依赖于数据的判别函数的好处，其更好地处理多模式/异构数据;此外，该方法提供更多不同的稀疏网络响应，这反过来可以用于成本有效的学习/分类。网络是通过结合传统的softmax和每层提出的熵损失来学习的。用于决策树生长的熵损根据类别标签分布来度量数据激活的纯度。所提出的损失函数的反向传播规则来源于CNN的随机梯度下降（SGD）优化。我们表明，我们提出的方法优于三个公共图像分类基准和一个人脸验证数据集的最先进的方法。我们还演示了辅助数据标签的使用（如果有的话），这有助于我们的方法学习更多的区别性路由和表示，并导致改进的分类。

##### URL
[https://arxiv.org/abs/1706.02003](https://arxiv.org/abs/1706.02003)

##### PDF
[https://arxiv.org/pdf/1706.02003](https://arxiv.org/pdf/1706.02003)
