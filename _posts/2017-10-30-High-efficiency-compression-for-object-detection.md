...
图像和视频压缩传统上是根据人类视觉而设计的。然而，可视化分析和监视等现代应用依靠计算机在人类之前（或代替人类）看到和分析图像。对于这些应用程序，将压缩调整为计算机视觉很重要。在本文中，我们提出了一个比特分配和速率控制策略，是针对对象检测。使用最先进的物体检测器的初始卷积层，我们创建一个重要图，可以将比特分配引导到对物体检测非常重要的区域。与默认的HEVC相比，所提出的方法能够以相等的目标检测率节省7％或更多的比特率。
