...
FusionGAN是一个融合了生成对抗网络和双重学习优势的新一代音乐融合框架。具体而言，所提出的方法提供了双重学习扩展，可以有效地整合给定域的样式。为了有效量化不同域之间的差异并避免消失梯度问题，FusionGAN提供基于Wasserstein的度量来近似目标域和现有域之间的距离。采用Wasserstein距离，通过结合使用敌对学习的现有领域的模式创建一个新的领域。公共音乐数据集的实验结果表明，我们的方法可以有效地合并两种流派。
