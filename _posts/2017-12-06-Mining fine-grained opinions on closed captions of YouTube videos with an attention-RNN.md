...
视频评论是书面产品评论的自然演变。在本文中，我们针对这一现象，介绍了从YouTube产品评论视频的隐藏式字幕创建的第一个数据集，以及用于方面提取和联合方面提取和情感分类的新的注意 -  RNN模型。我们的模型在方面提取方面提供了最先进的性能，而不需要在SemEval ABSA语料库上使用手工特征，而在联合任务上则优于基线。在我们的数据集中，注意 -  RNN模型胜过两个任务的基线，但是与SemEval相比，我们观察到所有模型的重要性能下降。这些结果以及关于方面提取领域适应性的进一步实验表明，在文献中已经广泛讨论的语音和书面文本之间的差异也延伸到产品评论的领域，其中它们与细粒度意见挖掘。
