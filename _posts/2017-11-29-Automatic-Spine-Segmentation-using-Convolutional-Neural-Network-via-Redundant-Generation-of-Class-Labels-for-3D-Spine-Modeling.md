...
2010年至2016年，脊柱问题患者人数大幅增加。从计算机断层扫描（CT）图像获得的脊柱的自动图像分割对于诊断脊柱状况和用计算机辅助手术系统进行手术是重要的。脊柱有一个复杂的解剖结构，由33个椎骨，23个椎间盘，脊髓和连接肋骨组成。因此，脊柱外科医生面临着需要强大的算法来分割和创建脊柱模型的挑战。在这项研究中，我们开发了一种自动分割方法来分割脊柱，并将我们的分割结果与专家获得的参考分割进行了比较。我们开发了一种基于混合方法的CT全自动脊柱分割方法。该方法将卷积神经网络（CNN）和完全卷积网络（FCN）相结合，利用类冗余作为软约束条件，大大提高了分割结果。所提出的方法被发现显着提高了分割结果的准确性和系统处理时间。我们的比较基于12个测量值：Dice系数（94％），Jaccard指数（93％），体积相似性（96％），灵敏度（97％），特异性（99％），精确度精度（99％），Matthews相关系数（0.93），平均表面距离（0.16 mm），Hausdorff距离（7.4 mm）和总体一致性误差（0.02）。我们对32例患者的CT图像进行了实验，实验结果证明了所提方法的有效性。
