...
date: 2017-11-29 16:42:52
categories: arXiv_SD
tags: arXiv_SD Object_Detection Detection Recognition
...
现有的音乐识别应用程序需要连接到执行实际识别的服务器。在本文中，我们将介绍一种低功耗的音乐识别器，它完全在移动设备上运行，并自动识别音乐，而无需用户交互。为了减少电池消耗，小型音乐检测器在移动设备的DSP芯片上持续运行，并且只有在确信音乐存在时才唤醒主应用处理器。一旦唤醒，应用处理器上的识别器被提供有几秒钟的指纹音频，并且与存储在成千上万首歌曲的设备指纹数据库中的指纹进行比较。我们提供的系统“正在播放”，平均每天的电池使用量不到1％，通过完全在设备上运行并尊重用户隐私，可以被动地识别各种音乐。
