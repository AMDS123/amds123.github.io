...
date: 2017-06-09 10:50:43
categories: arXiv_CV
tags: arXiv_CV Ontology CNN Image_Classification Transfer_Learning Classification Recognition
...
MirBot是智能手机的协作应用程序，允许用户执行对象识别。这个应用程序可以用来拍摄一个物体的照片，选择感兴趣的区域，并通过使用从卷积神经网络（CNN）中提取的特征进行相似性搜索来获得最有可能的类别（狗，椅子等）。系统提供的答案可以由用户验证，以改善未来查询的结果。所有图像都与一系列元数据一起存储，从而实现了WordNet本体中用synset标识符标记的多模式增量数据集。这个数据集不断增长，得益于用户的反馈，并公开发表研究。本工作详细介绍了MirBot对象识别系统，分析了使用四年多以来收集的统计数据，描述了图像分类方法，并使用手工特征，卷积神经编码和不同的传输学习技术进行了详尽的评估。通过比较各种模型和转换方法，结果表明，尽管新类别的数量不断增加，CNN特征保持了MirBot随时间的恒定精度。该应用程序在Apple和Google Play商店免费提供。

##### URL
[https://arxiv.org/abs/1706.02889](https://arxiv.org/abs/1706.02889)

##### PDF
[https://arxiv.org/pdf/1706.02889](https://arxiv.org/pdf/1706.02889)
