...
视觉注意力是一个具有相当历史的领域，眼动控制和预测是一个重要的子领域。在过去的几十年里，固定建模在很大程度上是由一些非常有影响力的自下而上的显着性模型（如Itti-Koch-Niebur模型）在计算上主导的。由于深入的学习，这些模型的精确度近期大幅提高。然而，在静态图像上，这些模型的重点基本上是基于通过显着图来对图像进行无序预测。实施的模型很少能够产生超出最初注视点的时间上有序的人类扫视序列。为了解决这些缺点，我们提出了STAR-FC，这是一种基于深度学习显着性和低级特征显着性的中央/外围集成的新型多扫视发生器。我们使用CAT2000数据库评估了我们的模型，与使用一个人类序列预测另一个人类序列所能达到的等同的准确性和质量，成功地预测了人类的固定模式。相比于最先进的显着性算法预测的固定序列，这是一个显着的改进。
