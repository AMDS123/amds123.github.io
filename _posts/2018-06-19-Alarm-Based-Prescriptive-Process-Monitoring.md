...
预测过程监控涉及对执行过程中产生的事件进行分析，以预测其正在进行的案例的未来状态。该领域的现有技术能够预测，在案件的每个步骤中，案件将以不期望的结果结束的可能性。然而，这些技术并没有考虑到流程工作者可能会如何处理所产生的预测结果，以减少产生不良后果的可能性。本文提出了规范过程监控框架，该框架以警报，干预，补偿和缓解影响的概念扩展了预测过程监控方法。该框架结合了参数化成本模型，以评估在给定环境中应用指定过程监控的成本收益折衷。本文还概述了一种在给定数据集和一组成本模型参数的情况下优化警报生成的方法。所提出的方法是使用一系列实际事件日志经验性评估的。

##### URL
[http://arxiv.org/abs/1803.08706](http://arxiv.org/abs/1803.08706)

##### PDF
[http://arxiv.org/pdf/1803.08706](http://arxiv.org/pdf/1803.08706)
