...
本文的目的是学习口语理解（SLU）中的槽填充任务的跨域表示。最近发布的大多数SLU模型都是针对特定领域的，适用于各个任务域。注释每个单独的任务域的数据在财务上是昂贵的且不可扩展的。在这项工作中，我们提出了一种对抗性的培训方法来学习可以跨多个领域共享的共同特征和表征。产生这种共享表示的模型可以与在单个域SLU数据上训练的模型相结合，以减少开发新域所需的训练样本的数量。在我们使用来自多个领域的数据集的实验中，我们显示对抗训练有助于学习更好的领域一般SLU模型，从而提高填充F1得分。我们进一步表明，在领域一般模型上应用敌对学习也有助于在模型与领域特定模型联合优化时获得更高的填充性能。
