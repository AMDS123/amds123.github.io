...
冷漠是一类用于控制基于奖励的代理人的方法，例如，通过安全地改变他们的奖励或政策，或者使代理人表现得好像某种结果永远不会发生一样。这些控制方法即使不能完全理解代理人报酬的含义，也能起作用。尽管他们都是出于类似的想法，冷漠技术可以被分类为实现三个不同目标中的一个或多个的方式：依赖于某些事件的奖励（没有动机代理人操纵这些事件的可能性），有效的怀疑一个事件将会发生，并且从一个行为到另一个行为的无缝过渡。实现这三个目标有五个基本的方法。本文对POMDP中的这些方法进行了分类和分析（虽然这些方法对于其他代理设计来说是高度可移植的），并且确定了它们的用途，优势和局限性。它旨在使代理设计人员对冷漠的工具通常可以访问和使用。

##### URL
[http://arxiv.org/abs/1712.06365](http://arxiv.org/abs/1712.06365)

##### PDF
[http://arxiv.org/pdf/1712.06365](http://arxiv.org/pdf/1712.06365)
