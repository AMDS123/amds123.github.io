...
真实世界的图像识别系统需要识别数以万计的构成过多视觉概念的类。在这种情况下，传统的为每个类别注释数千个图像的方法是不可行的，这促使使用网络监督的数据。本文探讨了大量图像和相关用户评论的图像识别系统的培训。特别是，我们开发可以预测与图像内容相关的任意短语的可视化n元模型。我们的视觉n元模型是前馈卷积网络，使用新的损失函数进行训练，受到语言建模中常用的n元模型的启发。我们证明了我们的模型在短语预测，基于短语的图像检索，相关图像和字幕以及零点转移方面的优点。
