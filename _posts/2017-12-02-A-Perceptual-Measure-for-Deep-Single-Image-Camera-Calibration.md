...
目前大多数单幅图像相机校准方法依赖于特定的图像特征或用户输入，并且不能应用于在不受控制的设置下捕捉的自然图像。我们提出使用深度卷积神经网络从单个图像直接推断相机校准参数。该网络使用自动生成的大规模全景数据集样本进行训练，并且在标准L2误差方面远远优于其他方法，包括最近的基于深度学习的方法。然而，我们认为，在很多情况下，考虑人类如何在相机估计中感知错误更为重要。为此，我们进行了一次大规模的人类感知研究，要求用户判断三维物体是否合成了地面真实照相机标定的真实性。基于这项研究，我们开发了一种新的摄像机标定的感知测量，并证明我们的深度标定网络胜过了其他方法。最后，我们演示了如何使用我们的校准网络进行虚拟对象插入，图像检索和合成等多种应用。
