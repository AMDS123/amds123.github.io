...
date: 2017-06-16 07:14:01
categories: arXiv_CV
tags: arXiv_CV Object_Detection Survey Detection
author: Hsing-Wen Lin, Ying-Tung Chen, Jen-Hung Wang, Shiang-Yu Wang, Fumi Yoshida, Wing-Huen Ip, Satoshi Miyazaki, Tsuyoshi Terai
mathjax: true
---
...
机器学习技术被广泛应用于许多现代的光学天空调查，eqq。 Pan-STARRS1，PTF / iPTF和Subaru / Hyper Suprime-Cam调查，以减少人为干预进行数据验证。在这项研究中，我们已经建立了一个基于机器学习的真实假人系统，以排除斯巴鲁/超级超级凸轮战略调查项目（HSC-SSP）源目录中的错误检测。因此，HSC-SSP运动物体检测流水线可以更有效地运行，因为误报的减少。为了训练真实的虚拟系统，我们使用固定源作为真正的训练集，“标记”数据作为假集。训练集包含47个特征，其中大部分是从HSC图像缩小管道（hscPipe）产生的光度测量和形状力矩。我们的系统在fpr〜5％时可以达到真正的阳性率（tpr）〜96％，假阳性率（fpr）〜1％或tpr〜99％。因此，我们得出结论，固定光源是正确的实际训练样本，使用光度测量和形状时刻可以有效地排除假阳性。
