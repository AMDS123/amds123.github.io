...
相对姿态估计是计算机视觉中的一个基本问题，几十年来一直研究传统的全局快门相机。然而，近来，卷帘式快门相机由于其低成本的成像能力而被广泛使用，并且由于卷帘式快门相机逐行捕获图像，所以卷帘式快门相机的相对姿态估计比全球快门相机。在本文中，我们建议利用惯性测量（重力和角速度）进行卷帘快门相对姿态估计问题。惯性测量提供关于两个视图（照相机）之间的部分相对旋转的信息和引起卷帘快门失真的瞬时运动。基于这些信息，我们简化了卷帘快门相对姿态估计问题，并提出了有效的解决方法。不像以前的方法，需要44（线性）或17（非线性）点与统一滚动快门相机模型，所提出的方法至多需要9或11点来估计滚动快门相机之间的相对位置。综合数据和公共PennCOSYVIO数据集的实验结果表明，所提出的方法优于现有的方法。
