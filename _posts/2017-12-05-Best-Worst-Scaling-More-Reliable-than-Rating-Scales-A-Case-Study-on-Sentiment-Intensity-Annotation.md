...
评分量表是一种广泛使用的数据注释方法;然而，他们提出了几个挑战，例如难以保持内部和内部的注释一致性。最佳最差缩放（BWS）是另一种注释方法，声称可以产生高质量的注释，同时保持所需的注释数量与评定量表相似。但是，这一说法的真实性从来没有系统地确定过。在这里，我们第一次建立了一个直接比较评分尺度方法和BWS的实验。我们表明，在注释总数相同的情况下，BWS产生的结果比评定量表更可靠。
