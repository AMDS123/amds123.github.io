---
layout: post
title: "Avaliac{c}~ao da doenc{c}a de Alzheimer pela an'alise multiespectral de imagens DW-MR por redes RBF como alternativa aos mapas ADC"
...
阿尔茨海默病是痴呆的最常见原因，但是如果不使用侵入性技术，特别是在疾病开始时，难以准确诊断。这项工作解决了由扩散加权磁共振脑体积组成的多光谱合成图像的分类和分析，以评估脑脊液的面积及其与阿尔茨海默病的进展的相关性。 1.5T磁共振成像系统被用来获取所有的图像。分类方法基于径向基函数网络的多层感知器和分类器。假定感兴趣的类可以用超等式分开。一个二次多项式网络被用来分类原始的卷，产生一个地面实况卷。分类结果被用来改善通常的表观扩散系数分析。
