...
最近，四元数协同表示分类（QCRC）和四元数稀疏表示分类（QSRC）已经被提出用于彩色人脸识别。他们可以获得不同颜色通道之间的相关信息。但是，在不同的条件下它们的性能是不稳定的例如，QSRC在某些情况下的表现要好于QCRC，但在其他情况下表现更差。为了受益于QCRC中基于四元数的$ e_2 $ -norm最小化以及QSRC中基于四元数的$ e_1 $ -norm最小化，我们提出了基于四元数的自适应表示（QAR），它使用基于四元数的$ e_p $ -norm最小化（$ 1 \ le p \ le 2 $）用于彩色人脸识别。为了获得不同颜色通道间的高维相关信息，进一步提出了基于高维四元数的自适应表示（HD-QAR）。实验结果表明，提出的QAR和HD-QAR比QCRC，QSRC和几种最先进的方法获得更好的识别率。
