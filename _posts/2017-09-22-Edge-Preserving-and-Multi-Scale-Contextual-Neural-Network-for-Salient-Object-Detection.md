...
在本文中，我们提出了一种新颖的边缘保持和多尺度上下文神经网络用于显着物体检测。拟议的框架旨在解决现有CNN方法的两个限制。首先，基于区域的CNN方法缺乏足够的上下文来准确地定位显着对象，因为它们独立处理每个区域。其次，基于像素的CNN方法由于存在卷积和积层而遭受模糊的边界。受此启发，我们首先提出了一种基于快速R-CNN框架（命名为RegionNet）的端到端边缘保留神经网络，以高效生成尖锐物体边界的显着图。后来为了进一步完善，RegionNet考虑了区域与全球场景之间的关系。此外，我们的方法可以普遍应用于深度细化的RGB-D显着性检测。所提出的框架首次同时实现了清晰的检测边界和多尺度的上下文鲁棒性，从而达到了优化的性能。在六个RGB和两个RGB-D基准数据集上的实验证明了所提出的方法达到了最先进的性能。
