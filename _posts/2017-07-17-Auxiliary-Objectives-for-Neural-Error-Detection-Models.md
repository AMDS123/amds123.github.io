...
我们调查了不同辅助目标和训练策略在神经序列标记方法中对学习者写作中的错误检测的效用。辅助成本为模型提供了额外的语言信息，使其能够学习通用的组合特征，然后可以将其用于其他目标。我们的实验表明，在域内数据上使用并行标签训练的联合学习方法比以前的最佳错误检测系统提高了性能。虽然结果模型具有相同数量的参数，但额外的目标可以更有效地进行优化，并实现更好的性能。
