...
我们提出了一种有效利用对象信息的语义部分检测方法。我们使用对象外观及其类作为指示部件的期望。我们还根据其外观对物体内部件的预期相对位置进行建模。我们通过一个名为OffsetNet的新网络模块来实现这一功能，该模块可高效地预测给定对象内可变数量的零件位置。我们的模型结合了所有这些线索来检测物体背景下的零件。与单独使用零件外观（PASCAL-Part数据集上的+5 mAP）相比，这对于零件检测具有挑战性的任务具有相当高的性能。我们也比较了PASCAL-Part和CUB200-2011数据集上的其他部分检测方法。
