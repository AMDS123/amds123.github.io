...
categories: arXiv_CV
tags: arXiv_CV Image_Caption Object_Detection Attention Caption RNN Detection Relation
...
自动生成图像的自然语言描述是接近图像理解的核心任务。在本文中，我们提出了一种与人类视觉系统密切相关的多模型神经网络方法，自动学习来描述图像的内容。我们的模型由两个子模型组成：一个对象检测和定位模型，分别提取对象的信息和它们在图像中的空间关系;此外，还提出了一种基于长时间短记忆（LSTM）单元的深度递归神经网络（RNN），其具有句子生成的注意机制。描述中的每个单词在生成时将自动与输入图像的不同对象对齐。这与人类视觉系统的注意机制相似。 COCO数据集上的实验结果显示了该方法的优点，该方法优于以前的基准模型。
