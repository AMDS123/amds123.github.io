...
生成图像的自然语言描述是机器人或其他视觉智能驱动的人工智能代理的重要功能，可能需要与人类用户就其所看到的信息进行交流。这种图像字幕方法通常通过最大化给定图像的地面真实注释字幕的可能性来训练。虽然简单易用，但这种方法并不能直接最大化我们关心的语言质量指标，如CIDEr。在本文中，我们研究了基于行为者 - 评论者强化学习的训练图像字幕方法，以直接优化不可区分的感兴趣的质量度量。通过在这个基于强化学习的新字幕模型中制定一个每令牌优势和价值计算策略，我们表明可以在广泛使用的MSCOCO基准上达到最高水平的性能。
