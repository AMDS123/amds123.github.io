...
继2016年4月在美国陆军研究实验室（ARL）和南加州大学创新技术研究所（USC ICT）之间正式名为ARL West的一项倡议之后，ARL在2016年夏天进行了一次现场实验的协调和执行， USC ICT和Headwall Photonics。其目的是为美国洛杉矶的USC主校区的一部分，使用两个便携式CO​​TS（商业现成的）空中无人机解决方案进行数据采集，进行摄影测量（从图像进行三维重建），以及将高光谱数据与恢复的代表目标区域的3D点云。该研究旨在确定具有能够将目标区域分割成关键材料类别（例如，人造结构，活植物，水）以用于多种目的的机器的可行性，包括为用户提供更准确的场景理解和实现从目标区域的有意义的材料类别的无监督的自动采样，用于自适应半监督机器学习。在后者中，一个目标集库可以用于自动机器训练，以当地物质类别的数据为例，增加机器识别目标的预测机会。现场实验和相关的数据后处理方法，以纠正反射，校正，从图像中恢复区域的密集点云，从独立收集的数据集中记录具有场景表面的高程特性的光谱，并生成期望的场景分割图。整个文件中也强调了从经验中吸取的经验教训。
