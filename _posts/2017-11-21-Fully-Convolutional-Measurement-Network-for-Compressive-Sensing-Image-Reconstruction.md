...
最近，深度学习方法在压缩感知图像重建任务中取得了重大进展。但是，这仍然是一个块效应的问题，会降低重建结果。在本文中，我们提出了一个完全卷积网络，其中完整的图像直接用卷积层进行测量。由于重叠的卷积测量，块效应被删除。另外，由于测量和重建阶段的联合训练，可以获得自适应测量。此外，为了提高网络的性能，在重建网络中使用剩余学习。实验结果表明，所提出的方法在PSNR和视觉效果上均优于现有方法。
