...
在本文中，我们提出了一个鲁棒跟踪器来解决多目标跟踪（MOT）问题，在跟踪检测的框架下。作为第一个贡献，我们创新性地将单目标跟踪（SOT）算法与多目标跟踪算法相结合，并且我们的结果表明，无论检测质量如何，SOT都是大大减少漏报数量的一般方法。另外一个贡献是我们展示了一个深度的学习型外观模型，很容易将同一个对象的检测有效地联系起来，而且也很准确。此外观模型在我们的MOT算法中将检测正确地与长轨迹联系起来，并且在我们的SOT算法中发现被检测器错误地遗漏的新检测。基于深度神经网络的模型确保了我们的跟踪算法的鲁棒性，可以在各种场景中执行数据关联。我们在一个大型的具有挑战性的数据集MOT16基准测试中进行了全面的实验，结果显示我们的跟踪器基于公共和私人检测达到了最先进的性能。
