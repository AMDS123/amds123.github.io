...
图像字幕是一项重要而又具有挑战性的任务，适用于虚拟助手，编辑工具，图像索引和残障人士的支持。其挑战是由于可能的图像描述的可变性和模糊性。近年来，使用长时间短记忆（LSTM）单元的递归神经网络在图像字幕方面取得了重大进展。尽管减轻了渐变梯度问题，并且尽管它们具有强大的记忆依赖性的能力，但是LSTM单元在时间上是复杂且固有顺序的。为了解决这个问题，最近的工作已经显示了卷积网络在机器翻译和条件图像生成方面的好处。受到他们的成功启发，本文研究了一种卷积图像字幕技术。我们在具有挑战性的MSCOCO数据集上展示其功效，并展示与基线相媲美的性能，同时每个参数的训练时间更快。我们也进行了详细的分析，提供了令人信服的理由来支持卷积语言生成方法。
