...
盲增益和相位校准（BGPC）是一个双线性反问题，涉及确定传感系统的未知增益和相位以及未知信号。 BGPC出现在许多应用中，例如反演中的盲反照率估计，合成孔径雷达自动对焦和传感器阵列自动校准。在某些情况下，未知信号的稀疏结构缓解了BGPC的不适应性。最近，对BGPC的解决方案有了新的兴趣，仔细分析了误差范围。在本文中，我们将BGPC作为特征值/特征向量问题，并提出通过功率迭代法，或在稀疏或联合稀疏情况下，通过截断幂迭代来求解。在一定的假设下，未知增益，相位和未知信号可以同时恢复。数值实验表明，功率迭代算法不仅适用于我们主要结果预测的方案，而且适用于理论分析有限的方案。我们还展示了我们的BGPC的功率迭代算法与对抗条件下的竞争算法（例如，具有噪声测量或具有不好的初始估计）相比有利。
