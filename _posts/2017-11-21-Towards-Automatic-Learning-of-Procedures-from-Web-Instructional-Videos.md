...
通过观察执行涉及对象和行为的程序的其他代理来学习代理人（无论是具体还是软件）的潜力是很丰富的。目前关于自动程序学习的研究在很大程度上依赖于动作标签或视频字幕，甚至在评估阶段，这使得它们在现实世界中不可行。这导致了我们的问题：程序的人类共识结构是否可以从大量长时间，无限制的视频（例如来自YouTube的教学视频）中学习，只有视觉证据？为了回答这个问题，我们介绍了过程分割的问题 - 将视频过程分割成与范畴无关的过程段。鉴于这个问题没有大规模的数据集可用，我们收集了一个大规模的程序分割数据集，其中程序段被临时定位和描述;我们使用烹饪视频，并命名数据集YouCook2。我们提出了一个段级循环网络来生成过程段通过建模跨段的依赖关系。生成的段可以用作其他任务的预处理，例如密集的视频字幕和事件解析。我们在实验中表明，所提出的模型在程序分割方面胜过竞争基线。
