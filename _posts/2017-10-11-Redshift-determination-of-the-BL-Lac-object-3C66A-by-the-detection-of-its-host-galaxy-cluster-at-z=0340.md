...
BL Lac目标3C66A是TeV $ \ gamma $ -rays（VHE，即$ E> 100 $ GeV）中最明亮的河外源之一。由于TeV $ \ gamma $射线辐射被河外背景光（EBL）吸收，因此了解源的红移以重建其原始光谱能量分布以及约束EBL模型是至关重要的。然而，这个BL \，Lac的光谱几乎没有特征，所以直接测量$ z $是非常困难的;事实上，这个来源（$ z = 0.444 $）已发布的红移值已受到强烈质疑。基于EBL吸收论据，提出了在$ 0.096 <z <0.5 $的范围内对其红移的若干限制。由于这些AGN通常位于组群或团簇早期型星系，所以我们通过分光光度计对3C66A的环境进行了分析，目标是寻找银河系中的银河系。这项研究是使用以blazar为中心的$ 5.5 \ times 5.5 $ \ arcmin $ ^ {2} $的光学图像，以及使用Gemini / GMOS-N多目标光谱学获得的24个光源的光谱。我们在blazar的视线中发现了两个星系团的光谱证据：一个在$ z \ simeq 0.020 $，另一个在$ z \ simeq 0.340 $。第一个与已知的前景结构相一致，而第二个小组则提供了六个光谱确认的成员。他们沿着颜色大小图中的红色序列的位置使我们能够确定另外34个候选成员。 blazar的光谱显示了我们认为在星系间介质中产生的广泛的吸收特征，因此我们可以暂时将红移下限设定为$ z_ {3C66A} \ ge 0.33 $。因此，我们建议将3C66A托管在属于$ z = 0.340 $的集群的星系中。
