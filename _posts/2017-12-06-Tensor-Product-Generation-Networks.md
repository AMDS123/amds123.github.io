...
我们提出了一个新的张量产品生成网络（TPGN），为图像生成自然语言描述。该模型具有一种新颖的架构，通过神经网络计算来实例化用于编码和处理符号结构的通用框架。该框架建立在张量产品表示（TPR）上。我们评估了在MS COCO图像字幕任务上提出的TPGN。实验结果表明，TPGN优于基于LSTM的现有技术基准，并具有显着的余量。此外，我们显示我们的字幕生成模型可以被解释为生成语法类别的序列，并从编码为分布式表示的计划的类别中检索单词。
