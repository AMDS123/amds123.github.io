...
我们引入了多种模型，在监督图像字幕语料上进行训练，预测给定字幕的图像特征，进行句子表示的基础。我们训练了一个可以在COCO字幕和图像检索上获得良好性能的接地句子编码器，并且随后显示这个编码器可以成功地转换到各种NLP任务，并且改进了纯文本模型的性能。最后，我们分析了接地的贡献，表明这个系统学习到的字嵌入优于非接地的嵌入。
