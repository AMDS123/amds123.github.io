...
categories: arXiv_CV
tags: arXiv_CV Portrait_Generation Image_Generation
...
素描人像生成广泛应用，包括数字娱乐和执法。尽管现有的人脸素描生成方法取得了很大的进展，但是它们大多产生了模糊的效果，并且在各种脸部部分上变形很大为了解决这个问题，我们提出了一个新的组合辅助生成对抗网络（CA-GAN）来生成素描肖像。首先，我们利用成对的输入，包括人脸照片和相应的逐像素人脸标签来生成人像。其次，我们提出了一种改进的像素损失，称为成分损失，以将训练集中在难以生成的组件和精致的面部结构上。此外，我们使用堆栈CA-GAN（stack-CA-GAN）来进一步纠正缺陷并增加引人注目的细节。实验结果表明，我们的方法能够产生身份保持，素描逼真，视觉舒适的素描肖像在广泛的挑战性数据，并且胜过现有的方法。此外，我们的方法显示出相当大的泛化能力。
