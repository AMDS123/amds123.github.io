...
categories: arXiv_CL
tags: arXiv_CL Embedding RNN Classification
...
从给定的文本表达句子的挖掘建议是一个较少调查的句子分类任务，因此缺乏手标记的基准数据集。在这项工作中，我们提出并评估两种建议挖掘远程监督的方法。遥远的监督是通过一个大银标准数据集，使用wikiHow和维基百科的文本构建而成。这两种方法都使用基于LSTM的神经网络体系结构来学习建议挖掘的分类模型，但在使用银级标准数据集的方法上有所不同。第一种方法是使用这个数据集直接训练分类器，而第二种方法只是从这个数据集中学习单词嵌入。在第二种方法中，我们也学习POS嵌入，这有趣地给出了最好的分类准确性。
