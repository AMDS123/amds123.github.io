...
实体链接（EL）中的一个主要挑战是有效利用上下文信息来消除维基百科中提及的可能涉及不同上下文中的不同实体的提及。这个问题会加剧跨语言EL，这涉及到将非英语文档中的提及与英文维基百科中的条目链接起来：比较跨语言的文本线索，以计算不同语言的文本片段之间的相似性。在本文中，我们提出了一个神经EL模型，从多个角度，结合卷积和张量网络，训练查询和候选文档之间的细粒度的相似性和不相似性。此外，我们表明，这种英语培训的系统可以通过令人惊讶地有效使用多语言嵌入，在零点学习中应用于其他语言。所提出的系统具有强大的经验证据，可以产生最新的英语和跨语言结果：西班牙语和中文TAC 2015数据集。
