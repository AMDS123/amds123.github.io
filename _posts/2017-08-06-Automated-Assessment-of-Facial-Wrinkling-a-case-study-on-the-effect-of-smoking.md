...
伴随自然衰老过程的面部皱纹是最显着的生物变化之一。但是，有一些外部因素会导致过早的皱纹发展，例如日晒和吸烟。临床研究表明，严重的吸烟会导致过早的皱纹发展。但是，没有可以自动评估整个面部皱纹的计算机化系统。本研究使用社交习惯人脸数据集和自动计算机化的计算机视觉算法来调查吸烟对面部起皱的影响。首先使用改进的混合Hessian滤波器提取以0-255的强度表示的皱纹图案。脸被分成十个预定义的区域，每个区域的皱纹都被提取出来。然后进行统计分析，分析哪个区域主要受吸烟影响。结果显示，嘴周围两个地区吸烟者的皱纹密度显着高于非吸烟者，p值为0.05。其他地区由于缺乏大规模的数据集是不确定的。最后，通过生成通用3D脸部模型，在吸烟者和非吸烟者面部之间在视觉上比较皱纹。

##### URL
[https://arxiv.org/abs/1708.01844](https://arxiv.org/abs/1708.01844)

##### PDF
[https://arxiv.org/pdf/1708.01844](https://arxiv.org/pdf/1708.01844)
