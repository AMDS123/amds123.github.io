...
categories: arXiv_CV
tags: arXiv_CV CNN Classification Deep_Learning Quantitative
...
本文处理了估算细丝网络如视网膜血管和道路网络的拓扑结构的任务。建立在对图像像素进行密集语义分类的全局模型之上，我们设计了一个卷积神经网络（CNN），用于预测输入补丁的中心像素与其边界点之间的局部连通性。通过迭代这个局部连通性，我们扫描整个图像，并推断丝状网络的全局拓扑结构，灵感来自人类用手指尖描绘复杂的网络。我们对两个任务进行了广泛和全面的定性和定量评估：视网膜静脉和动脉拓扑提取和道路网络估计。在两个公开的数据集（DRIVE和马萨诸塞州道路）中，我们都展示了非常强大的基线的优越性能。
