...
同步定位和映射（SLAM）研究越来越趋向于更丰富的世界表征，涉及对象和高级特征，使得机器人世界的语义模型成为可能，导致更有意义的机器人世界交互。其中许多进步都是基于最先进的计算机视觉技术，这些技术主要是在基于图像的基准数据集的背景下开发的，留下了几个挑战，以适应机器人技术的应用。在本文中，我们推导了一个同时定位和映射（SLAM）的公式，它使用双二次函数作为三维地标表示，并且显示二维边界框（例如通常从视觉对象检测系统获得的边界框）是如何直接约束二次参数的。我们的论文演示了如何在基于因子图的SLAM中利用通用透视相机来联合估计机器人姿态和双二次参数，并且涵盖了使用单眼相机移动的机器人的使用情况，并且没有附加深度信息的可用性。
