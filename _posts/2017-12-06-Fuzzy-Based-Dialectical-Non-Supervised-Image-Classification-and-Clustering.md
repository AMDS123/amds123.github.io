...
唯物辩证法是分析现实的一种哲学的调查方法。这些方面被看作是由名为极点的基本单位组成的复杂过程，它们相互作用。辩证法在黑格尔的辩证法和20世纪的马克思，恩格斯和葛兰西哲学和经济学的着作中，在19世纪取得了长足的进步。两极之间矛盾的运动被看作是一个动荡的过程，伴随着进化和革命危机的交织。为了建立一个基于辩证法的计算过程，极点之间的相互作用可以用模糊隶属函数来模拟。基于这个假设，我们引入了目标辩证分类器（ODC），这是一个基于唯物辩证法的非监督分类图，作为模糊c均值分类器的扩展。作为一个案例研究，我们使用ODC对181个由质子密度，$ T_1 $和$ T_2 $加权合成脑图像组成的磁共振合成多光谱图像进行分类。将ODC与k-means，fuzzy c-means和Kohonen自组织映射进行比较，证明ODC能够达到与Kohonen自身最优的非监督分类器几乎相同的量化性能组织的地图。
