...
本文提出了一种实时嵌入式跌倒检测系统，该系统使用从未被用于传统跌倒检测的DVS（动态视觉传感器），用于跌倒检测的数据集以及DVS-TN（DVS-时间网络）。第一个贡献是建立DVS瀑布数据集，使我们的网络能够比之前存在的现有数据集识别更多种类的瀑布，并利用DVS解决隐私问题。其次，我们介绍DVS-TN：优化的深度学习网络，使用DVS来检测跌倒。最后，我们实现了一个能实时运行在低运算硬件上的跌倒检测系统，并在考虑了各种跌倒情况的DVS跌倒数据集上进行测试。我们的方法在F1分数上达到95.5％，在NVIDIA Jetson TX1板上以31.25 FPS运行。
