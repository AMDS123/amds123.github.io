...
视觉和语言的交叉问题对于具有挑战性的研究问题和丰富的应用程序都是非常重要的。然而，我们世界的固有结构和我们的语言中的偏见往往是一个比视觉形式更简单的学习信号，导致忽视视觉信息的模型，导致其能力的膨胀感。我们建议针对视觉问答（VQA）的任务对付这些语言的先验，并使视觉（VQA中的V）重要！具体而言，我们通过收集互补图像来平衡流行的VQA数据集，使得我们的平衡数据集中的每个问题不仅与单个图像相关联，而且与一对相似的图像相关联，从而导致两个不同的问题的答案。我们的数据集的结构比原始的VQA数据集更加平衡，大约有两个图像问题对的数量。我们完整的平衡数据集可以在www.visualqa.org上公开获取，作为视觉问答应答数据集和挑战（VQA v2.0）的第二次迭代的一部分。我们在平衡的数据集上进一步对许多最先进的VQA模型进行基准测试。所有的模型在我们的平衡数据集上都表现得更差，这表明这些模型确实学会了利用语言先验。这个发现提供了第一个具体的经验证据，在实践中似乎是一种质的意义。最后，我们用于识别补充图像的数据收集协议使我们能够开发一种新颖的可解释模型，除了提供给定（图像，问题）对的答案之外，还提供了基于反例的解释。具体来说，它确定了一个与原始图像相似的图像，但它相信对于同一个问题有不同的答案。这可以帮助在用户之间建立对机器的信任。
