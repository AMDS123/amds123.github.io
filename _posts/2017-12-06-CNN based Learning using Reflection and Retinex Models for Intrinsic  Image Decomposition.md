...
大部分传统的固有图像分解工作都依赖于获取关于场景特征的先验知识。另一方面，最近的研究将深度学习模式作为进出黑箱，并没有将完善的传统图像形成过程作为其内在学习过程的基础。因此，尽管目前的深度学习方法在考虑量化基准测试结果时表现出优异的性能，但传统方法在取得高质量结果方面仍占主导地位。本文的目的是要挖掘两个世界中最好的一个。 （1）具有深度学习能力，（2）考虑基于物理学的反射模型来引导学习过程，（3）利用传统方法通过利用反射率和阴影梯度信息获得固有图像。所提出的模型是快速计算，并允许所有内在组件的集成。为了训练新模型，创建了一个以内在地面真实图像为中心的大规模数据集。评估结果表明，新模型优于现有方法。目视检查显示图像形成损失函数增强了色彩再现，并且使用梯度信息产生更锐利的边缘。

##### URL
[https://arxiv.org/abs/1712.01056](https://arxiv.org/abs/1712.01056)
