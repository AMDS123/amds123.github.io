...
野外的图像封装了丰富的关于各种抽象概念的知识，并且不能用仅使用包含选定对象的图像 - 标题对构建的模型来充分描述。我们建议在一个包含许多抽象概念的知识库的指导下处理这样的任务。我们的方法是一个两步过程，我们首先建立一个多实体标签的图像识别模型来预测抽象概念作为图像标签，然后利用它们作为外部语义注意和约束推理在字幕生成模型中的第二步描述描绘看不见的/新颖的物体的图像。评估表明，我们的模型胜过了大多数以前的关于MSCOCO域外字幕的工作，对整体知识和视觉的整合是有用的。
