...
序列到序列（Seq2Seq）模型在涉及生成机器翻译，图像字幕和语音识别等自然语言句子的任务方面具有突出的优势。通过利用无标签的数据（通常以语言模型的形式）来进一步提高性能。在这项工作中，我们提出冷融合方法，利用训练过程中的预训练语言模型，并显示其在语音识别任务的有效性。我们表明，与冷聚变Seq2Seq模型能够更好地利用语言信息享受i）更快的收敛和更好的泛化，和ii）几乎完成转移到一个新的领域，而使用少于10％的标记的训练数据。
