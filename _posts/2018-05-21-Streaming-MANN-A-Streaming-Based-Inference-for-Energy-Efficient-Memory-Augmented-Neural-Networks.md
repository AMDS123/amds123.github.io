...
随着使用深度学习的人工智能的成功开发，对其部署的兴趣越来越大。移动环境是最接近现实生活的硬件平台，已成为人工智能成败的重要平台。存储器增强神经网络（MANN）是提出的有效处理问答（Q＆A）任务的神经网络，非常适合移动设备。由于MANN需要各种类型的操作和循环数据路径，因此难以加速为其他传统神经网络模型设计的结构中的推断，这是在移动环境中部署MANN的最大障碍之一。为了解决上述问题，我们提出了流媒体MANN。这是第一次尝试使用流处理的概念来实现和演示MANN的节能推理架构。为了充分发挥流媒体过程的潜力，我们提出了一种新的方法，称为推理阈值，使用贝叶斯方法考虑自然语言处理（NLP）任务的特征。为了评估我们提出的方法，我们在现场可编程门阵列（FPGA）中实现了适用于流处理的架构和方法。我们测量了bAbI数据集推断的执行时间和功耗。实验结果表明，与NVIDIA TITAN V的结果相比，Streaming MANN的每能量性能效率（FLOPS / kJ）增加了约126倍，如果应用推理阈值则增加到140。
