...
图像分割是将图像分割成易于分析的重要区域的过程。如今，分割已经成为许多实际医学成像方法中定位肿瘤和疾病的必要条件。隐马尔可夫随机场模型是用于图像分割的几种技术之一。它提供了一个优雅的方式来模拟分割过程。这种建模导致了目标函数的最小化。共轭梯度算法（CG）是最着名的优化技术之一。本文提出使用共轭梯度算法（CG）进行图像分割，基于隐马尔可夫随机场。由于导数不可用于此表达式，因此CG算法中使用有限差分来近似一阶导数。该方法使用许多公开可用的图像进行评估，其中地面实况已知。 Dice系数被用作衡量分割质量的客观标准。结果表明，所提出的CG方法与其他隐马尔可夫随机场分割算法的变体相比更有优势。

##### URL
[http://arxiv.org/abs/1705.04823](http://arxiv.org/abs/1705.04823)

##### PDF
[http://arxiv.org/pdf/1705.04823](http://arxiv.org/pdf/1705.04823)
