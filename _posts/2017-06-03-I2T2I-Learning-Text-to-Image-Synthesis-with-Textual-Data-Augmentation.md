...
在文本和图像之间转换信息是连接自然语言处理和计算机视觉的人工智能的基本问题。在过去的几年里，通过采用递归神经网络（RNN），图像标题生成的性能得到了显着的改善。同时，文本到图像的生成开始使用鸟类和花朵等特定类别的数据集生成合理的图像。我们甚至通过使用生成对抗网络（GAN），从多类别数据集（例如Microsoft通用对象上下文（MSCOCO））中生成图像。然而，合成具有复杂形状的物体仍然具有挑战性。例如，动物和人类有许多自由度，这意味着他们可以承担许多复杂的形状。我们提出了一种称为图像 - 文本 - 图像（I2T2I）的新型训练方法，它集成了文本到图像和图像到文本（图像字幕）综合，以提高文本到图像合成的性能。我们证明了我们的方法理解句子描述的能力，使得I2T2I可以使用MSCOCO生成比现有技术更好的多类图像。我们还证明，I2T2I可以通过使用预先训练的图像字幕模块在MPII人体姿态上生成人体图像来实现转移学习
