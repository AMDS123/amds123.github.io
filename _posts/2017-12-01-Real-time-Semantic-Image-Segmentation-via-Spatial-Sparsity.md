...
我们提出了一种语义（图像）分割方法，将计算成本降低了25倍，对结果质量的影响有限。语义分割具有许多实际应用，对于大多数这样的应用来说，计算成本是关键的。该方法遵循典型的双列网络结构，其中一列接受输入图像，而另一列接受该图像的半分辨率版本。通过识别全分辨率图像中可以安全忽略的特定区域，以及仔细地修改网络结构，我们可以使用单个GTX 980视频卡每秒处理大约15个高分辨率Cityscapes图像（1024x2048），同时达到平均值在Cityscapes测试集上交集72.9％的交集。
