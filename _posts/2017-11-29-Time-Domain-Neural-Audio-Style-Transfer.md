...
最近发布的音频风格转换方法展示了如何将图像风格转换的过程扩展到音频。该方法使用短时傅立叶变换，具有随机初始化滤波器的浅卷积网络和使用Griffin-Lim的迭代相位重构的大小独立地合成音频“内容”和“样式”。在这项工作中，我们探讨是否有可能直接优化时域音频信号，消除相位重构的过程，为实时应用和更高质量的合成开辟了可能性。我们在直接对时域音频信号进行操作的神经网络上探索了各种样式的传输过程，并展示了一个能够实现音频程式化的网络。
