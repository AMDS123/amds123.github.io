...
categories: arXiv_CV
tags: arXiv_CV Sparse Attention Classification Recognition
...
基于表示的分类方法如基于稀疏表示的分类（SRC）和线性回归分类（LRC）已经引起了很多的关注。为了获得更好的表示效果，本文提出了一种新的基于投影表示的分类方法（PRC）。中国是基于一个新的数学模型。这个模型表示在超空间$ H $上的一个样本点$ x $的“理想投影”可以通过迭代计算$ x $在一个超空间$ H $上的投影来获得，并且具有适当的策略。因此，PRC能够迭代地逼近每个主题的“理想表示”进行分类。此外，还提出了判别式PRC（DPRC），其通过最大化类间重构误差与类内重建误差的比率来获得判别信息。对五个典型数据库的实验结果表明，提出的PRC和DPRC在多个视觉识别任务上是有效的并且胜过了其他最先进的方法。

##### URL
[https://arxiv.org/abs/1712.01643](https://arxiv.org/abs/1712.01643)

##### PDF
[https://arxiv.org/pdf/1712.01643](https://arxiv.org/pdf/1712.01643)
