...
基于局部图像特征的方法近来已经显示出对于纹理分类任务的希望，特别是在由于照明，尺度和视点变化而存在大的类内变化的情况下。受到图像结构分析理论的启发，本文提出了一种简单，有效，鲁棒的描述子，即纹理分类的局部喷墨模式（LJP）。在这种方法中，纹理图像的喷气空间表示是从高斯（DtGs）滤波器响应达到二阶的一组导数（LJV）导出的，其也满足比例空间特性。通过利用中心像素与喷气空间中的局部邻域信息的关系来获得LJP。最后，通过将LJP的所有元素的LJP的直方图级联，形成纹理区域的特征向量。所有的二阶DtGs响应一起保留了固有的局部图像结构，实现了尺度，旋转和反射的不变性。这使我们能够开发出具有判别力和鲁棒性的纹理分类框架。对Outex_TC-00010（Outex_TC10）和Outex_TC-00012（Outex_TC10）进行五个标准纹理图像数据库的大量实验，采用最近子空间分类器（NSC），所提出的描述符达到100％，99.92％，99.75％，99.16％和99.65％ Outex_TC12），KTH-TIPS，Brodatz，CUReT，这些都超越了最新的方法。
