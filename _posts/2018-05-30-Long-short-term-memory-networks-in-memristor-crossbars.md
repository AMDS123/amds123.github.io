...
最近在具有长短期记忆（LSTM）单元的复发性深度神经网络方面的突破已经导致人工智能的重大进步。然而，具有显着增加的复杂性和大量参数的最先进的LSTM模型由于有限的存储器容量和数据通信带宽而导致计算能力的瓶颈。在这里，我们通过实验证明LSTM可以用忆阻器交叉开关实现，它具有较小的电路占用空间，可存储大量参数和内存计算能力，可以绕过“冯·诺伊曼瓶颈”。我们通过解决回归和分类中的实际问题来说明我们系统的能力，这表明忆阻器LSTM是一种用于边缘推理的有前途的低功耗和低延迟硬件平台。
