...
这项工作提出了一种新颖的方法稳健PCA与前景背景分离和去噪嘈杂，移动摄像机视频总变异正则化。我们提出的算法注册视频的原始（可能是损坏的）帧，然后联合处理注册的帧，以产生场景分解成低级背景分量，捕获场景的静态分量，平滑的前景分量捕获场景的动态组件以及隔离腐败的稀疏组件。与现有的方法不同，我们提出的算法产生一个横跨整个视野的全景低秩分量，自动地将来自部分重叠场景的损坏数据拼接在一起。我们稳健的PCA模型的低阶部分基于最近发现的最优低秩矩阵估计器（OptShrink），其不需要参数调整。我们演示了我们的算法在静态和移动摄像机视频中被噪声和异常值破坏的性能。
