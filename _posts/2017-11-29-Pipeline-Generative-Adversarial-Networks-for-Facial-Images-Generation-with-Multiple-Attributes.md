...
生成敌对网络被证明是有效的各种图像生成任务。但是，如果我们想要精确地生成图像，这仍然是一个挑战。许多研究人员专注于如何生成具有一个属性的图像。但是在多重属性下的图像生成仍然是一项艰巨的工作。在本文中，我们尝试使用流水线处理在多个约束条件下生成各种人脸图像。我们提出的Pip-GAN（管道生成对抗网络）采用了一种管道网络结构，可以使用中性人脸图像逐步生成复杂的人脸图像。我们在两个人脸图像数据库中应用了我们的方法，并展示了它能够在多种条件下生成令人信服的新颖图像。
