...
categories: arXiv_CV
tags: arXiv_CV Sparse Optimization Classification Deep_Learning Recognition
...
为了更好地选择正确的训练样本，并获得查询样本的鲁棒表示，本文提出了一种基于判别式的稀疏优化学习模型。这种学习模式将判别和稀疏结合在一起。基于此模型，我们提出了一种称为局部性判别式稀疏表示（LDSR）的分类器。由于判别式有助于增加不同类别样本的差异，减少同类样本的差异，因此LDSR可以获得更好的稀疏系数，构成更好的分类稀疏表示。为了充分利用核函数，判别式和稀疏性等优点，进一步提出了一种非线性分类器，称为核局部性判别式稀疏表示（KLDSR）。对几个知名数据库进行的实验证明，LDSR和KLDSR的性能优于包括深度学习在内的几种最先进的方法。
