...
categories: arXiv_CV
tags: arXiv_CV Attention CNN Classification Quantitative
...
反射共聚焦显微镜（RCM）是皮肤癌诊断的有效的非侵入性预筛查工具，但需要广泛的培训和经验来进行准确评估。图像采集和分析标准化的工具很少，可用的工具是不可解释的。在这项研究中，我们使用一个注意卷积网络特征的递归神经网络。我们将其应用于以可解释的方式描绘垂直定向的横向RCM图像切片中的皮层。我们引入一种新的注意机制，称为Toeplitz注意，它将注意力映射限制为具有Toeplitz结构。在专家标记的504个RCM堆栈数据集上测试我们的模型，我们实现了88.17％的图像分类精度，这是当前最先进的技术。
