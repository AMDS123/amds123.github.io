...
水下机器视觉吸引了更多的关注，但可怕的质量禁止它从广泛的应用。有很多算法来解决这个问题，但是实时性和自适应性方法是不足的。在本文中，基于滤波和生成对抗网络（GAN），针对上述问题提出了两种方法，即基于滤波的恢复方案（FRS）和基于GAN的恢复方案（GAN-RS）。与以往不同的是，FRS在频域恢复水下视觉，由参数搜索，滤波和增强组成。 GAN-RS能够进一步提高图像质量，无需任何预处理即可实时自适应清除水下机器视觉。特别是，$ Lab $颜色空间和暗通道中的信息被开发为损失函数，即水下指标损失和暗通道先前损失。更具体地说，从水下索引中学习，鉴别器配备精心制作的水下分支来预测图像的水下可能性。那么，多阶段损失策略就是为了保证GAN的有效培训而设计的。通过图像质量和应用的广泛比较，所提出的方法的优越性被证明。因此，GAN-RS实现了相当高的处理速度，并在可变水下场景的色彩校正，对比度拉伸和细节出现方面取得了进展。

##### URL
[https://arxiv.org/abs/1712.00736](https://arxiv.org/abs/1712.00736)
