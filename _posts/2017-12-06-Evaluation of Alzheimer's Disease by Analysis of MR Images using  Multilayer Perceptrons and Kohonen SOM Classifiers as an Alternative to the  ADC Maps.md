...
阿尔茨海默氏病是痴呆的最常见原因，但在没有侵入性技术的情况下难以确诊，特别是在疾病发作时。这项工作是对弥散加权磁共振（MR）脑图像合成的多光谱图像进行图像分析和分类，以评估脑脊液面积和测量阿尔茨海默病的进展。使用临床1.5T磁共振成像系统获取所有的图像。分类方法基于多层感知器和Kohonen自组织映射分类器。我们假设感兴趣的类可以通过超夸克来分离。因此，使用二维多项式网络对原始图像进行分类，生成地面真实图像。分类结果用于改善表观扩散系数图的通常分析。

##### URL
[https://arxiv.org/abs/1712.00712](https://arxiv.org/abs/1712.00712)
