...
我们建议使用神经网络（NN）来同时检测和定位人机相互作用（HRI）中的多个声源。与传统的信号处理技术不同，基于NN的声源本地化（SSL）方法相对简单，并且不需要或更少的假设难以在真实的HRI场景中保持。以前，基于神经网络的方法已经成功地应用于单一的SSL问题，在检测和本地化方面没有扩展到多个来源。在本文中，我们提出了一种基于似然的网络输出编码，这自然允许检测任意数量的源。此外，我们研究了使用子带交叉相关信息作为更好的声音混合中的本地化特征，以及基于不同处理动机的三种不同的NN体系结构。从机器人记录的实际数据的实验表明，我们的基于神经网络的方法显着优于流行的基于空间频谱的方法。
