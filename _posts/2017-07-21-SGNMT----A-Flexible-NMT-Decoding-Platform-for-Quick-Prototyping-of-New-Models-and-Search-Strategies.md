...
categories: arXiv_CL
tags: arXiv_CL Face NMT Language_Model
...
本文介绍了机器翻译研究的实验平台SGNMT。 SGNMT为神经和符号评分模块（预测指标）提供了从左到右的语义的通用接口，如NMT，语言模型，翻译格子，$ n $  -  best列表或其他类型的分数和约束等翻译模型。预测器可以与其他预测器组合，形成复杂的解码任务。 SGNMT实现了多种搜索策略来遍历由适合于不同预测器星座的预测器所跨越的空间。添加新的预测器或解码策略非常容易，使其成为一个非常有效的新工具原型设计工具。 SGNMT正在被剑桥大学的机器学习，语音和语言技术硕士课程中的学生积极使用，用于课程研究和论文以及我们大部分研究工作。
