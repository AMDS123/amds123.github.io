...
尽管剽窃源代码对于大多数CS学生来说是一项微不足道的任务，但检测这种不道德的行为需要付出相当大的努力。因此，开发了几个抄袭检测系统来处理这个问题。本文通过结合抽象方法线性化来扩展Karnalim的工作，这是一种检测Java源代码抄袭的低级方法。这种扩展是为了提高在面向对象环境下检测抄袭的低级方法的准确性。根据我们的评估，这是基于23个设计模式源代码对进行的，我们的扩展低级方法比现有技术和Karnalim的方法更有效。一方面，与最先进的方法相比，我们的方法可以产生较少的巧合相似性，并提供更准确的结果。另一方面，与Karnalim的方法相比，我们的方法在一定程度上可以在简单的抽象方法调用被引入时产生更高的相似性。
