...
信息在神经网络中传播的方式非常重要。在本文中，我们提出了路径聚合网络（PANET），旨在提高基于提议的实例分割框架中的信息流。具体来说，我们通过自下而上的路径增强在较低层中使用精确的定位信号来增强整个特征层次，这缩短了较低层和最顶层特征之间的信息路径。我们提出自适应特征池，它链接特征网格和所有特征级别，以使每个特征级别中的有用信息直接传播到后面的提议子网。创建针对每个提议捕获不同视图的补充分支以进一步改进掩模预测。这些改进很容易实现，具有微妙的额外计算开销。我们的PANet在COCO 2017 Challenge Instance Segmentation任务中排名第一，在没有大批量培训的情况下在对象检测任务中排名第二。它也是MVD和Cityscapes的最新技术。代码可从https：// github获得。 COM / ShuLiu1993 / PANet。
