...
在整个面部表情中，肌肉活动在特定的持续时间内引起某些AU的激活。本文提出了使用激活肌肉的面部特征的面部距离来识别面部动作单元（AU）的方法。涉及的七个脸部动作单元是AU1，AU4，AU6，AU12，AU15，AU17和AU25，表征快乐和悲伤的表情。根据基于每个脸部点的距离定义的规则对每个AU进行识别。所选择的面部距离是从十二个面部特征中提取的。然后使用支持向量机（SVM）和神经网络（NN）训练面部距离。分类结果使用支持向量机提出了几个不同的支持向量机内核，而结果使用神经网络提出了每个培训，验证和测试阶段。
