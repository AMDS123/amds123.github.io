...
在最先进的神经机器翻译（NMT）中，在解码期间使用关注机制来增强翻译。在每一步中，解码器都使用这种机制来聚焦源句子的不同部分，以在输出其目标词之前收集最有用的信息。最近，关注机制的有效性也在多模式任务中得到了探索，在这种模式下，可以把注意力集中在他们描述的句子部分和图像区域上。本文比较了多模式翻译任务（英语，图像到德语）的几种注意机制，并评估了模型利用图像来提高翻译的能力。我们在Multi30k数据集上超越了最先进的分数，但是我们在翻译的时候识别和报告机器的不同行为。
