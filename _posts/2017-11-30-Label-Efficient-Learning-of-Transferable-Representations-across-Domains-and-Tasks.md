...
我们提出了一个框架，学习可以跨标签有效地跨不同领域和任务转让的代表。我们的方法与域对抗损失战斗域转移，并使用基于度量学习的方法将嵌入到新任务概括。我们的模型同时针对标记的源数据和目标域中未标记或稀疏标记的数据进行了优化。我们的方法在新领域内的新类别上显示出令人瞩目的结果，即使每个类别只有少数几个标注的例子可用，也超越了普遍的微调方法。此外，我们还展示了从图像对象识别到视频动作识别的转换学习任务框架的有效性。
