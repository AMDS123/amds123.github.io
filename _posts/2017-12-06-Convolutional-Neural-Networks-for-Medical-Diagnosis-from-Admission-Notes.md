...
##### Abstract
$\textbf{Objective}$ Develop an automatic diagnostic system which only uses textual admission information from Electronic Health Records (EHRs) and assist clinicians with a timely and statistically proved decision tool. The hope is that the tool can be used to reduce mis-diagnosis. 
...
$ \ textbf {Objective} $开发一个自动诊断系统，它只使用电子健康记录（EHRs）的文本录取信息，并协助临床医生获得及时和统计学证明的决策工具。希望这个工具可以用来减少误诊。
 $ \ textbf {材料和方法} $我们使用MIMIC-III的实际临床记录，这是一个免费的数据库，包含四万多名住在Beth Israel Deaconess医疗中心的重症监护病房的病人的临床资料2001年和2012年。我们提出了卷积神经网络模型来学习非结构化文本输入的语义特征，并自动预测主要流出诊断。
 $ \ textbf {Results} $所提出的模型在10个最常见的疾病类别上实现了总体的96.11％的准确率和80.48％的加权F1分值，在加权的F1分值上显着优于四个强的基线模型至少12.7％。
 $ \ textbf {Discussion} $实验结果意味着CNN模型适用于在存在复杂，噪声和非结构化临床数据的情况下支持诊断决策制定，同时使用其他传统Deep Network模型的更少的层和参数。
 $ \ textbf {结束语}我们的模型展示了从非结构化临床记录和常见误诊常见疾病的预测能力中表现复杂医学意义特征的能力。它可以很容易地在临床上采用，提供及时和统计学证明的决策支持。
 $ \ textbf {Keywords} $卷积神经网络，文本分类，排放诊断预测，电子健康档案收录信息。

##### URL
[http://arxiv.org/abs/1712.02768](http://arxiv.org/abs/1712.02768)

##### PDF
[http://arxiv.org/pdf/1712.02768](http://arxiv.org/pdf/1712.02768)
