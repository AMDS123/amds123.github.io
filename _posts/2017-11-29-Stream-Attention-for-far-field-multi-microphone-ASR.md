...
流注意框架已被应用于深度神经网络（DNN）的后验概率，以改善多麦克风配置中的远场自动语音识别（ASR）性能。流注意方案通过注意向量来实现，注意向量是通过从单个话筒流的音素后验分布预测ASR性能而得到的，从而将识别器的注意力集中到更可靠的话筒上。已经使用真实记录的数据集对各种ASR性能测量进行了调查。实验结果表明，所提出的框架已经在字错误率（WER）方面取得了实质性的改进。
