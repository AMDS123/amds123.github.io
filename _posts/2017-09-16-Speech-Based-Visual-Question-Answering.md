...
本文介绍了基于语音的视觉问答（VQA），即给出图像和口头问题的答案。研究了两种方法：直接使用音频波形作为输入的端到端深度神经网络与对问题执行ASR（自动语音识别）的流水线方法，然后是基于文本的视觉问题回答。此外，我们通过在口头问题中注入各种级别的噪音来研究这两种方法的稳健性，并发现两种方法都能够在相似的级别上忍受噪音。
