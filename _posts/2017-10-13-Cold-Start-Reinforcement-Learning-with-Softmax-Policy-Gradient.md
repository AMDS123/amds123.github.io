...
用于强化学习的策略梯度方法有两个常见和不期望的开销过程，即热启动训练和样本方差减少。在本文中，我们描述了一个基于softmax值函数的强化学习方法，它不需要这些程序。我们的方法结合了策略梯度方法的优点和最大似然方法的效率和简单性。我们将这种新的冷启动强化学习方法应用于结构化输出预测问题的训练序列生成模型。经验证据验证了这种方法在自动汇总和图像字幕任务。
