...
图像字幕是一项具有挑战性的任务，它结合了计算机视觉和自然语言处理领域。已经提出了各种方法来实现自动描述图像的目标，并且基于递归神经网络（RNN）或长短期记忆（LSTM）的模型主导该领域。但是，RNN或LSTM不能并行计算并忽略句子的基本层次结构。在本文中，我们提出了一个框架，它只使用卷积神经网络（CNN）来生成字幕。由于并行计算，我们的基本模型在培训期间比NIC（基于LSTM的模型）快约3倍，同时还提供更好的结果。我们对MSCOCO进行了大量实验，并研究了模型宽度和深度的影响。与采用类似注意机制的基于LSTM的模型相比，我们提出的模型实现了BLEU-1,2,3,4和METEOR的可比分数，以及更高的CIDEr分数。我们还在段落注释数据集上测试我们的模型，并且与分层LSTM相比获得更高的CIDEr得分
