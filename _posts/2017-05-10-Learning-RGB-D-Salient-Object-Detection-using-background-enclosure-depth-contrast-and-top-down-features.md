...
最近，深度卷积神经网络（CNN）在RGB显着物体检测上表现出了强大的性能。虽然深度信息可以帮助改善检测结果，但是探测用于RGB-D显着目标检测的CNN仍然是有限的。在这里，我们提出了一种深度CNN架构的RGB-D显着物体检测，利用高级别，中级和低级别的功能。此外，我们提出新颖的深度特征，捕捉适合于学习方法的背景外壳和深度对比的想法。与最先进的RGB-D显着物体检测方法相比，我们显示出改进的结果。我们还表明，低层和中层深度特征都有助于改善结果。尤其是，我们的方法F-Score在RGBD1000数据集上是0.848，比第二位好10.7％。
