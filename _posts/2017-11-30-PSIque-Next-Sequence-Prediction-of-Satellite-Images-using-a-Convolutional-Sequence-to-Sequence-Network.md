...
预测未知的天气现象是灾难管理的一个重要问题。在本文中，我们提出了一个卷积序列自动编码器的模型，用于预测以前卫星图像中未被发现的天气情况。我们还提出了编码器和解码器模块之间的对称跳跃连接，以产生更全面的图像预测。为了检验我们的模型性能，我们对每个建议的模型进行了实验，以预测未来卫星图像的卫星图像。跳跃连接和序列到序列自动编码器的特定组合能够从地面实况图像产生最接近的预测。
