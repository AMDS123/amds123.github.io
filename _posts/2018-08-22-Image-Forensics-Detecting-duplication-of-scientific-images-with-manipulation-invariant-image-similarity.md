...
在科学出版物中操纵和重复使用图像是一个令人关注的问题，目前缺乏可扩展的解决方案。目前用于检测图像复制的工具大多是手动或半自动的，尽管可用于基于学习的方法的压倒性目标数据集。本文通过复制，旋转，平移，缩放，透视变换，直方图调整或部分擦除来解决确定给定两个图像是否是另一个的操纵版本的问题。我们提出了一种基于3分支暹罗卷积神经网络的数据驱动解决方案。对ConvNet模型进行训练，将图像映射到128维空间，其中重复图像之间的欧几里德距离小于或等于1，并且独特图像之间的距离大于1.我们的结果表明这种方法具有有可能改进对已出版和同行评审文献的图像处理监督。
