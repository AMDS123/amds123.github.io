...
恢复3D全身人体姿势对于许多应用来说是一个具有挑战性的问题。运动捕捉系统已经成功解决了身体磨损标记和多个摄像头的问题。在本文中，我们解决了更具挑战性的情况，不仅使用单个摄像头，而且还没有使用标记：直接从2D外观到3D几何。深度学习方法已经显示了显着的异能学习2D外观特征的能力。缺失的部分是如何整合二维，三维和时间信息来恢复三维几何，并解决了由判别模型引起的不确定性。我们引入了一种将二维关节位置作为潜变量的新方法，其不确定性分布由深度完全卷积网络给出。未知3D姿态用稀疏表示建模，3D参数估计通过期望最大化算法实现，其中表明2D关节位置不确定性可以在推理期间方便地被边缘化。基准数据集的广泛评估表明，所提出的方法比现有技术的基线更高的准确性。值得注意的是，所提出的方法不需要用于训练的同步2D-3D数据，并且适用于MPII数据集所证明的“野外”图像。

##### URL
[https://arxiv.org/abs/1701.02354](https://arxiv.org/abs/1701.02354)

##### PDF
[https://arxiv.org/pdf/1701.02354](https://arxiv.org/pdf/1701.02354)
