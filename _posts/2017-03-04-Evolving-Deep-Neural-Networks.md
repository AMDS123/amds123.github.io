...
深度学习的成功取决于找到适合任务的架构。随着深度学习扩展到更具挑战性的任务，架构变得难以手工设计。本文提出了一种自动化方法CoDeepNEAT，通过进化优化深度学习架构。通过将现有的神经元演化方法扩展到拓扑，组件和超参数，该方法在对象识别和语言建模的标准基准中实现了与最佳人体设计相媲美的结果。它还支持在杂志网站上构建一个真实的自动图像字幕应用程序。考虑到可用计算能力的预期增长，深度网络的演进是未来构建深度学习应用的有前途的方法。
