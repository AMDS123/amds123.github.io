...
最近，生成对抗网络（GAN）已经对包括低分辨率（LR）图像合成在内的广泛的应用产生了巨大的影响。但是，他们受到不稳定的训练，尤其是在图像分辨率提高的时候为了克服这个瓶颈，本文将最先进的Wasserstein GANs（WGANs）推广到能量放松的目标，从而实现更稳定和更高分辨率的图像生成。这种概括的好处可以总结为三个要点。首先，所提出的EnergyWGAN目标保证了有效的对称分歧，作为一个更加严格和有意义的定量度量。其次，EnergyWGAN能够搜索比原来的WGAN更忠实的解决方案空间，而不需要确定特定的$ k $ -Lipschitz约束条件。最后，所提出的EnergyWGAN提供了一种自然的方法来叠加GAN来生成高分辨率的图像。在我们的实验中，我们不仅证明了所提出的EnergyWGAN的稳定的训练能力，并且在标准基准数据集上获得了更好的图像生成结果，而且还显示了在现实世界的高分辨率图像数据集上优于最先进的GAN的优势。
