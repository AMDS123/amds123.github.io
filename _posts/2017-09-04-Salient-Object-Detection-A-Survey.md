...
在自然场景中检测和分割显着物体，通常称为显着物体检测，引起了人们对计算机视觉的很大兴趣。虽然已经提出了许多模型，并且出现了一些应用，但对成就和问题还没有深入的理解。我们的目标是全面回顾突出显示目标检测的最新进展，并将这一领域置于其他密切相关领域，如通用场景分割，目标提议生成和固定预测的显着性。涵盖228个出版物，我们调查i）根源，关键概念和任务，ii）核心技术和主要建模趋势，以及iii）显着物体检测中的数据集和评估指标。我们还讨论了开放性问题，如评估指标和模型性能的数据集偏差，并提出未来的研究方向。
