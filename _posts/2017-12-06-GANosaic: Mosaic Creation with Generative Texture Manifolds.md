...
本文提出了一个用卷积神经网络生成纹理马赛克的新框架。我们的方法被称为GANosaic并且在生成纹理模型的潜在噪声空间中执行优化，其允许将内容图像变换成展现下面的纹理流形的视觉属性的镶嵌图。为了表示这种流形，我们使用最先进的生成对抗方法进行纹理合成，从而可以从数据中学习表达纹理表示，并以非常高的分辨率生成马赛克图像。这个完全卷积模型生成平滑（没有任何可见的边界）马赛克图像，在本地变形和混合不同的纹理。另外，我们还开发了一种适用于PSGAN模型的先前噪声空间的新型可微分统计正则化。
