...
最近，人们对自动生成图像描述感兴趣。大多数现有的基于语言模型的方法用于该任务，学习按照其原始字词顺序逐字地生成图像描述。然而，对于人类来说，首先找到对象及其关系是比较自然的，然后详细阐述每个对象，描述显着的属性。本文提出了一种将原始图像描述分解为骨架句及其属性的粗到细的方法，并分别生成骨架句和属性短语。通过这种分解，我们的方法可以产生比先前的现有技术更准确和新颖的描述。在MS-COCO和更大规模的Stock3M数据集上的实验结果表明，我们的算法在不同的评估指标上产生了一致的改进，特别是在SPICE指标上，与传统指标相比，SPICE指标与人类评分的相关性更高。此外，我们的算法可以生成不同长度的描述，受益于骨架和属性的单独控制。这使得能够更好地适应用户偏好的图像描述生成。
