...
串行切片电子显微镜（ssEM）图像的对齐对于寻求重建神经元电路的神经科学的努力至关重要。但是，每个ssEM平面都包含密集的结构，从一个部分到下一个部分都是不同的，这使得整个图像的匹配特征成为一个挑战。深度学习的进展已经在相似的计算机视觉问题上产生了前所未有的性能，但就我们所知，它们还没有被成功应用于ssEM图像的共同注册。在本文中，我们引入了一个新的深层网络模型，结合了图像变形的空间变换和卷积自动编码器进行无监督特征学习，以实现稳健的ssEM图像对齐。与传统方法相比，这导致精确度和鲁棒性的改进，同时需要显着较少的用户干预。我们通过比较多个数据集的注册质量来评估我们的方法。
