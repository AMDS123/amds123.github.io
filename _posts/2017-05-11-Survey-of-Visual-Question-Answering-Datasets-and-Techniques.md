...
视觉问题回答（或VQA）是一个结合了自然语言处理和计算机视觉技术的令人兴奋的新问题。我们介绍了用于解决这一任务的各种数据集和模型。调查的第一部分详细介绍了VQA的各种数据集，并将其与一些常见因素进行了比较。本次调查的第二部分详细介绍了VQA的不同方法，分为非深度学习模式，不注意深度学习模式，注意力深度学习模式和不适合前三种模式。最后，我们比较这些方法的表现，并为今后的工作提供一些方向。
