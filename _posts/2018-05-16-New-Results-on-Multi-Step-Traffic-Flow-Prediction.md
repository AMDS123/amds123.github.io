...
在最简单的形式中，交通流量预测问题仅限于预测未来的单个时间步骤。多步骤交通流量预测将这种设置扩展到基于某些有限历史来预测未来多个时间步骤的情况。这个问题比其单步变体要困难得多，并且随着时间步长的增加，已知会受到预测退化的影响。在本文中，介绍了两种方法来改进递归和多输出设置中的多步骤交通流量预测性能。具体而言，引入了一种允许递归预测方法在进行预测时考虑时间步长索引时间上下文的模型。另外，为了改善多输出设置的预测性能，提出了一种有条件的生成对抗网络数据增强方法。在实际交通流量数据集上的实验表明，这两种方法分别改进了递归和多输出设置下的多步骤交通流量预测。

##### URL
[http://arxiv.org/abs/1803.01365](http://arxiv.org/abs/1803.01365)

##### PDF
[http://arxiv.org/pdf/1803.01365](http://arxiv.org/pdf/1803.01365)
