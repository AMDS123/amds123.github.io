...
阿尔茨海默病破坏脑细胞，导致人们失去记忆力，精神功能和持续日常活动的能力。这是一种严重的神经性脑病，不能治愈，但早期发现阿尔茨海默病可以帮助正确治疗和预防脑组织损伤。阿尔茨海默病（AD）的检测和分类是具有挑战性的，因为有时区分阿尔茨海默氏病MRI数据的迹象可以在正常的老年人的健康脑MRI数据中找到。此外，还有相对少量的数据集可用于培训自动阿尔茨海默病检测和分类模型。在本文中，我们提出了一种新型的阿尔茨海默病检测和分类模型，使用大脑MRI数据分析。我们开发了深度卷积神经网络的集合，并且在开放获取系列影像研究（OASIS）数据集上展示出优越的性能。
