...
RANSAC是鲁棒优化中的重要算法，也是许多计算机视觉应用的中心构建模块。近年来，传统上手工制作的管道已经被深度学习管道所取代，这些管道可以以端到端的方式进行培训。然而，迄今为止，RANSAC并没有被用作这种深度学习管道的一部分，因为它的假设选择过程是不可区分的。在这项工作中，我们提出了两种不同的方法来克服这个限制。最有前途的方法是受强化学习的启发，即用一个概率选择来代替确定性假设选择，我们可以推导出预期损失w.r.t.到所有可学习的参数。我们将这种方法称为RANSAC的可区分对象。我们将DSAC应用于摄像机定位问题，到目前为止，深度学习在传统方法上还没有得到改善。我们证明，通过直接最小化输出相机姿态的预期损失，通过RANSAC进行鲁棒估计，我们实现了准确度的提高。将来，任何深度学习管道都可以使用DSAC作为强大的优化组件。

##### URL
[https://arxiv.org/abs/1611.05705](https://arxiv.org/abs/1611.05705)

##### PDF
[https://arxiv.org/pdf/1611.05705](https://arxiv.org/pdf/1611.05705)
