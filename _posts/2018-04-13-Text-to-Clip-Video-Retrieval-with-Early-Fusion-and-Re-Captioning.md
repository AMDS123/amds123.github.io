...
我们提出了一种能够基于自然语言查询从未修剪视频中检索剪辑的新方法。这种跨模态检索任务在视觉语义理解中起着关键作用，并且需要及时定位剪辑并计算它们与查询句子的相似性。当前的方法生成句子和视频嵌入，然后使用后期融合方法对它们进行比较，但这会忽略查询中的单词顺序并阻止更细粒度的比较。由于需要细粒度的多模态特征融合，我们提出了一种新颖的早期融合嵌入方法，它将单词级别的视频和语言信息结合起来。此外，我们使用密集视频字幕的逆任务作为辅助任务来改进学习嵌入。我们的完整模型将这些组件与高效的提案管道相结合，可以对潜在的视频片段进行精确定位我们对两个大型文本到剪辑数据集（Charades-STA和DiDeMo）进行了全面的实验验证，并使用我们的模型获得了最先进的检索结果。
