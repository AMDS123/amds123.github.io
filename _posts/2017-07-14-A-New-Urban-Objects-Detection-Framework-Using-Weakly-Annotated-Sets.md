...
城市信息学探索数据科学方法，根据数据密集处理不同的城市问题。应该探索大量的数据和数据，但这带来了重大的挑战。例如，尽管可以探索有强大的计算机视觉方法，但他们可能需要大量的注释数据集。在这项工作中，我们提出了一个新的方法来自动创建一个最小的手动注释对象识别系统。该方法背后的基本思想是使用大型城市的可用在线摄像机的大量输入数据集。使用现成的弱分类器来检测初始的一组感兴趣的城市要素（例如，汽车，行人，自行车等）。这种初始数据集经历了一个质量控制程序，随后用来对一个强分类器进行微调。质量控制和比较绩效评估被用作管道的一部分。我们评估基于监控摄像头的汽车检测方法。使用真实数据的实验结果表明，尽管失去了通用性，最终的检测器提供了针对所选照相机定制的更好的检测率。编程的机器人从24个在线城市摄像机（\〜300GB）收集了770个视频小时，这个摄像机已经被提供给建议的系统。我们的方法已经表明，该方法使用现成的算法相对于现有技术方法的回想（93％）几乎翻了一番。
