...
随着遥感采集技术的飞速发展，需要扩大和改进处理工具，以应对观测到的数据量和丰富度的增加。在遥感技术中，深度学习越来越受到关注，但取决于训练数据的质量。因此，本文提出了最近的深度学习方法，用于精细或粗糙的土地覆盖语义分割估计。测试了各种2D体系结构，并引入了一个新的3D模型，以共同处理数据的空间和光谱维度。这样的一组网络能够比较不同的频谱融合方案。此外，我们也评估使用“嘈杂的基本事实”（即过时和低空间分辨率标签）来训练和测试网络。
