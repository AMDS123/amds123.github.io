...
本文讨论了一个具有挑战性的问题 - 如何从一个视图输入生成多视图布图像。为了生成具有不同视角的逼真图像，我们提出了一种新的图像生成模型，称为VariGANs，它结合了变分推理和生成对抗网络（GANs）的优势。我们提出的VariGANs模型以粗到细的方式生成目标图像，而不是一次通过严重伪影。它首先执行变分推理以模拟对象的全局外观（例如，形状和颜色），并产生具有不同视图的粗略图像。在生成的低分辨率图像上进行条件处理，然后进行敌对学习以填充细节并生成与输入一致的细节的图像。在两个服装数据集MVC和DeepFashion上进行的大量实验已经证明，由我们的模型生成的新颖视图的图像比现有方法生成的图像更加合理，在全球外观更加一致以及更丰富和更清晰的细节方面。

##### URL
[https://arxiv.org/abs/1704.04886](https://arxiv.org/abs/1704.04886)

##### PDF
[https://arxiv.org/pdf/1704.04886](https://arxiv.org/pdf/1704.04886)
