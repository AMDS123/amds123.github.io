...
视频分析需要使用大量的数据。压缩感应允许使用原始信号的稀疏性的先验知识来减少表示视频所需的测量次数，但是它在设计矩阵上施加了一定的条件。贝叶斯压缩感知方法使用概率推理来放松传统方法的局限性，并允许包括关于信号结构的不同的先前知识。本文提出了两种贝叶斯压缩感知方法，用于静态摄像机的视频序列中的自主对象检测。他们的性能在真实数据集上与非贝叶斯贪婪算法进行比较。结果表明，贝叶斯方法可以提供与贪婪算法相同的精度，但速度更快;或者如果计算时间不重要，则可以提供更准确的结果。
