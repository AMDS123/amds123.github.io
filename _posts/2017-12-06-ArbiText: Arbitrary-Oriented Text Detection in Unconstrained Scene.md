...
由于纵横比，比例尺，方向和照度的变化，任意导向的文本检测是一项非常具有挑战性的任务。在本文中，我们提出了一种新颖的方法，即任意导向的文本（或简称ArbText）检测器，在无约束的自然场景图像中进行高效的文本检测。具体来说，我们首先采用圆弧而不是矩形来表示边界框，这对于方向变化来说更加鲁棒。随后，我们将一个金字塔池模块加入到Single Shot MultiBox Detector框架中，以便同时探索本地和全球的视觉信息，从而可以产生更多的保密检测结果。在已建立的场景文本数据集（如ICDAR 2015和MSRA-TD500数据集）上进行的实验已经证明了与现有技术方法相比，所提出方法的最佳性能。
