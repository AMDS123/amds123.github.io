...
递归神经网络在视频字幕中的应用近来备受关注，因为它们既可以用来对输入视频进行编码，也可以用来生成相应的描述。在本文中，我们提出一个经常性的视频编码方案，可以发现和利用视频的层次结构。与经典的编码器 - 解码器方法（其中视频由递归层连续编码）不同，我们提出了一种新颖的LSTM单元，其可以识别帧或片段之间的不连续点并相应地修改编码层的时间连接。我们在三个大型数据集上评估我们的方法：蒙特利尔视频注释数据集，MPII电影描述数据集和Microsoft视频描述语料库。实验表明，我们的方法可以发现输入视频的适当的分层表示，并改善电影描述数据集上的艺术效果。
