...
三维几何数据为研究表示学习和生成建模提供了一个很好的领域。在本文中，我们看几何数据表示为点云。我们引入了具有最先进的重建质量和泛化能力的深度自动编码器（AE）网络。学习的表示比现有的三维识别任务的方法更好，通过简单的代数操作（如语义部分编辑，形状类比和形状插值）实现基本的形状编辑。我们对不同的生成模型进行了深入研究，包括：在原始点云上运行的GANs，在固定的潜在空间我们的AE和高斯混合模型（GMM）中显着改善的GANs。对于我们的定量评估，我们基于点云之间的匹配提出了样本保真度和多样性的度量。有趣的是，我们对泛化，保真度和多样性的仔细评估表明，在我们AE的潜在空间训练的GMM产生最好的结果。
