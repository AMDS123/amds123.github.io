...
新的看不见的类可以被形成为已知类的极端值。这激发了最近在开放式认可方面的工作{Scheirer_2013_TPAMI，Scheirer_2014_TPAMIb，EVM}，然而却无法命名这些看不见的新类。为了解决这个问题，我们提出极值学习（EVL）公式来学习从视觉特征到语义空间的映射。为了对每个班级的边际和覆盖率分布进行建模，词汇知情学习（ViL）通过在语义空间中使用广阔的开放词汇来采用。从本质上讲，通过结合EVL和ViL，我们首次提出了一种新的语义嵌入范式 - 词汇通知极值学习（ViEVL），它将视觉特征以概率的方式嵌入到语义空间中。学习嵌入可以直接用于解决监督学习，零点开放集合识别同时进行。在两个基准数据集上的实验证明了所提出的框架的有效性。

##### URL
[https://arxiv.org/abs/1705.09887](https://arxiv.org/abs/1705.09887)

##### PDF
[https://arxiv.org/pdf/1705.09887](https://arxiv.org/pdf/1705.09887)
