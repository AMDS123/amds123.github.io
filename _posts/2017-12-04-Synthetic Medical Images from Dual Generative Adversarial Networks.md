...
目前对数据驱动的医学图像分类方法非常感兴趣。但是，医学影像数据稀缺，昂贵，并且充满了关于患者隐私的法律问题。典型的同意书只允许将患者数据用于医学期刊或教育，这意味着大多数医学数据对于普通公众研究是不可访问的。我们提出了一个新的，两阶段的管道，用于从一对生成对抗网络生成合成医学图像，在实践中对视网膜基金图像进行测试。我们开发了一个分层生成过程，将复杂的图像生成任务分为两部分：几何和照片写实。我们希望研究人员能够利用我们的管道将私人医疗数据带入公共领域，从而激发先前依靠模型手动调整的成像任务的发展。我们已经开始通过SynthMed开发这个计划，SynthMed是一个合成医学图像的在线存储库。
