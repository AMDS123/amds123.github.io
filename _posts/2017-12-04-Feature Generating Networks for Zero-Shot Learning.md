...
由于看到的和看不见的类别之间极端的训练数据不平衡，现有的大多数现有技术方法对于具有挑战性的广义零点学习任务没有取得令人满意的结果。为了避免需要标签的例子看不见的类，我们提出了一种新的生成对抗网络（GAN）合成的CNN功能条件的类级别的语义信息，提供了一个直接从一个类的语义描述符到类条件功能分配。我们提出的方法，使Wasserstein GAN与分类损失相匹配，能够生成充分区分的CNN特征来训练softmax分类器或任何多模式嵌入方法。我们的实验结果表明，对于五个具有挑战性的数据集（CUB，FLO，SUN，AWA和ImageNet），在零射击学习和广义零射击学习设置中，相比现有技术的精确度显着提高。
