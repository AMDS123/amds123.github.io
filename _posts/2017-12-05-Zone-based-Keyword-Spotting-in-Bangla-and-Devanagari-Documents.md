...
在本文中，我们提供了一个文本行文字线查询系统，用于孟加拉（孟加拉）和梵文等离线印度文字。最近研究表明，区域识别方法在印度文字中比传统的全文字识别系统有更高的识别性能。受此启发，我们考虑使用区域分割方法，并使用中间区域信息来改善传统的字词识别性能。为了避免使用启发式方法进行区域分割的问题，我们在这里提出一种基于HMM的方法来从文本行图像中分割上下区域分量。候选关键字从一行中搜索而不用分割字符或单词。此外，我们提出了一种新颖的特征，结合文字线图像的前景和背景信息，用于字符填充模型的关键字识别。通过使用前景和背景信息比单独的信息显着改善性能。金字塔直方图的定向梯度（PHOG）功能已被用于我们的字词定位框架。从实验中已经注意到，所提出的基于区域分割的系统优于传统的词语识别方法。
