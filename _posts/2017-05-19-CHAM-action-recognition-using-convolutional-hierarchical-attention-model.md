...
最近，语言处理中最初提出的软注意机制已经应用于图像字幕等计算机视觉任务中。本文通过将卷积LSTM与分层系统架构相结合来识别视频中的动作类别，从而改进了软注意模型。我们称这个模型为卷积分级关注模型（CHAM）。该模型在LSTM单元内部应用卷积运算和注意映射生成过程来识别动作。该模型的分层体系结构能够明确地推断出多个行动类别的粒度。所提出的架构在三个公开可用的数据集上取得了改进的结果：UCF体育数据集，奥林匹克体育数据集和HMDB51数据集。
