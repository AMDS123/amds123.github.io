...
产品兼容性及其功能在客户购买产品时非常重要，在销售产品时对于销售商和制造商来说非常重要。由于网上有大量的产品可供使用，列举和测试每个产品的兼容性和功能是不可行的。在本文中，我们解决了两个紧密相关的问题：产品兼容性分析和功能可满足性分析，其中第二个问题是第一个问题的泛化（例如，产品是否与另一个产品一起工作可以被认为是特殊功能）。我们首先确定一个新的问题并回答有关产品兼容性和功能信息的最新的语料库。为了实现自动发现产品的兼容性和功能，我们提出了一个名为双注意网络（DAN）的深度学习模式。给定待购产品的QA对，DAN学习1）发现补充产品（或功能），2）准确预测发现产品（或功能）的实际兼容性（或可满足性）。模型解决的挑战包括QAs的简短性，表示兼容性的语言模式以及问题和答案的适当融合。我们进行实验来定量和定性地表明，与广泛的基线相比，所确定的产品和功能具有高覆盖率和准确性。

##### URL
[http://arxiv.org/abs/1712.02016](http://arxiv.org/abs/1712.02016)

##### PDF
[http://arxiv.org/pdf/1712.02016](http://arxiv.org/pdf/1712.02016)
