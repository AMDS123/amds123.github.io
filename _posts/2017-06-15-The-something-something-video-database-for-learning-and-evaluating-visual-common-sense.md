...
在数据集（如ImageNet）上训练的神经网络已经在视觉对象分类方面取得了重大进展。阻碍网络对复杂场景和情况进行更深入推理的障碍，以及像人类一样将视觉知识与自然语言相结合的一个障碍是他们缺乏关于物理世界的常识知识。与静止图像不同，视频包含大量有关物理世界的详细信息。然而，大多数有标签的视频数据集代表了高层概念，而不是关于动作和场景的详细物理方面。在这项工作中，我们描述了我们正在收集的视频预测任务的“某事”数据库，这些数据库的解决方案需要对所描述的情况有一个常识性的理解。该数据库目前包含超过174个类别的100,000多个视频，这些视频被定义为字幕模板。我们还描述了大规模集中这些数据所面临的挑战。
