...
大多数自然视频包含大量事件。例如，在一个“弹钢琴的人”的视频中，视频也可能包含“另一个人跳舞”或“人群鼓掌”。我们介绍密集字幕事件的任务，它涉及在视频中检测和描述事件。我们提出了一种新的模型，能够在一次视频中识别所有事件，同时用自然语言描述检测到的事件。我们的模型引入了现有建议模块的一个变体，该模块旨在捕获跨越几分钟的短时间和长时间事件。为了捕捉视频中事件之间的依赖关系，我们的模型引入了一个新的字幕模块，它使用来自过去和未来事件的上下文信息来共同描述所有事件。我们还介绍了ActivityNet字幕，这是一个关于密集字幕事件的大型基准。 ActivityNet字幕包含20万个视频，总计849个视频小时，总共有10万个描述，每个视频都有独特的开始和结束时间。最后，我们报告我们的密集字幕事件模型，视频检索和本地化的表现。
