...
在监督分类框架内，标记数据用于学习分类器参数。在此之前，通常需要通过特征提取来执行降维。这些预处理步骤激发了大量的研究工作，旨在恢复无监督环境中的潜在变量。本文提出了一个统一的框架来共同进行分类和低层建模。主要目标是使用估计的潜变量作为分类的特征，并且同时包含监督信息以帮助潜变量提取。所提出的分层贝叶斯模型分为三个阶段：第一个低级建模阶段，估计潜在变量，第二个阶段将这些特征聚类到统计学上的均质组，以及最后一个分类阶段利用（可能是严重的）标记数据。在高光谱图像解释的特定背景下评估模型的性能，统一两种标准分析技术，即解混和分类。
