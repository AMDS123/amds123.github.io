...
本文提出了一种密集语义对应的深层体系结构，称为金字塔仿射回归网络（PARN），用于估计图像中局部变化的仿射变换域。为了处理同一对象类别中不同实例之间通常存在的类内外观和形状变化，我们利用金字塔模型，其中以粗到细的方式逐步估计仿射变换场，以便自然地施加平滑约束。在深层网络中。 PARN估计每个级别的残差仿射变换并将它们组合以估计最终的仿射变换。此外，为了克服语义对应训练数据不足的局限性，我们提出了一种新颖的弱监督训练方案，该方案通过利用图像对之间的对应一致性来产生渐进监督。我们的方法可以以端到端的方式完全学习，并且不需要量化无限连续仿射变换场。据我们所知，这是第一个尝试在深度网络中以粗到细的方式估计密集仿射变换场的工作。实验结果表明，PARN在各种基准测试中优于最先进的密集语义对应方法。
