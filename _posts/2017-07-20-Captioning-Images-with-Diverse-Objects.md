...
最近的字幕模型在对配对的图像文本语料库中看不见的概念进行缩放和描述的能力方面受到限制。我们提出了新颖的对象字幕（NOC），一个深层的视觉语义字幕模型，可以描述大量的不存在于现有的图像 - 字幕数据集的对象类别。我们的模型利用外部来源 - 来自对象识别数据集的标记图像，以及从未注释文本中提取的语义知识。我们提出最小化联合目标，可以从这些不同的数据源学习，并利用分布式语义嵌入，使模型在图像标题数据集之外概括和描述新的对象。我们证明，我们的模型利用语义信息为ImageNet对象识别数据集中的数百个对象类别生成字幕，这些字幕在MSCOCO图像字幕训练数据中未被观察到，以及很少被观察到的许多类别。自动评估和人类判断都表明，我们的模型在描述更多类别的对象方面大大优于先前的工作。
