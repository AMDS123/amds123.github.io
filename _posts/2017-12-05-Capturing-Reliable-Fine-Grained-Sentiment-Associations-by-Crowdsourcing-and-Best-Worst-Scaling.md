...
访问字词情感关联对许多应用程序很有用，包括情感分析，立场检测和语言分析。然而，将细粒度的情感关联分值手动分配给单词在保持注释一致方面有许多挑战。我们应用最佳最差缩放的注释技术来获得三个不同领域（通用英语，英语Twitter和阿拉伯语Twitter）的单词和短语的实值情感关联分数。我们表明，在所有三个领域中，即使在用不同的注释器重复注释过程的情况下，由情绪来评估单词的排名仍然是非常一致的。我们还首次确定了一种语言的母语人士可以感受到的情感联想的最小差异。
