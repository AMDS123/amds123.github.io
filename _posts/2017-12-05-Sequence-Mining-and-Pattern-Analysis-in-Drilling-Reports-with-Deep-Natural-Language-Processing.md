...
石油和天然气行业的钻井活动已经有数十年的报道，每天都有数千口井，但是这个文本大规模分析信息检索，序列挖掘和模式分析是非常具有挑战性的。钻井报告包含钻井人员在井下传感器和地面设备上记录的测量结果的解释，可用于优化运营和减少事故。在这项初步工作中，提出了一种方法，用于将在钻探报告中写入的句子自动分类为在实际领域中的数百个井的三个相关标签（事件，症状和行动）。克服了文本语料库中的一些主要挑战，其中包括技术符号的高频率，技术术语的误写/缩写以及钻井报告中存在的不完整句子。我们使用这种技术语言获得了最先进的分类准确性，并说明了该工具提供的高级查询。
