...
date: 2017-07-20 07:07:33
categories: arXiv_CV
tags: arXiv_CV Object_Detection CNN Detection Relation Recognition
...
随着计算机视觉和机器学习应用的快速发展，可重复使用的模型设计将变得可取。在本文中，我们关注预先训练的深度卷积模型的可重用性。具体而言，与将预训练模型作为特征提取器处理不同，我们在卷积层下揭示更多的宝藏，即卷积激活可以充当图像共定位问题中的共同对象的检测器。我们提出了一种简单而有效的方法，称为深描述符变换（Deep Descriptor Transforming，DDT），用于评估描述符之间的相关性，然后得到类别一致性区域，这些区域可以准确地定位一组未标记图像中的共同对象，即无监督对象发现。实证研究验证了所提出的DDT方法的有效性。在基准图像共定位数据集上，DDT始终大大优于现有的最先进的方法。此外，滴滴涕还表现出良好的看不见的类别泛泛能力和鲁棒性处理嘈杂的数据。除此之外，还可以使用DDT将网页图像收集到有效的外部数据源中，以提高图像识别和对象检测的性能。

##### URL
[https://arxiv.org/abs/1707.06397](https://arxiv.org/abs/1707.06397)

##### PDF
[https://arxiv.org/pdf/1707.06397](https://arxiv.org/pdf/1707.06397)
