...
categories: arXiv_CV
tags: arXiv_CV Image_Caption Object_Detection Caption Embedding Language_Model Detection
...
使用显式对象检测器作为图像字幕的中间步骤 - 曾经构成早期工作的一个重要阶段 - 通常在当前占主导地位的端到端方法中被绕过，其中语言模型直接在中间 - 水平图像嵌入。我们认为显式检测提供了丰富的语义信息，因此可以用作可解释的表示，以更好地理解为什么端到端图像字幕系统运行良好。我们通过探索可以从这些对象检测中获得的各种提示，提供对端到端图像字幕的深入分析。我们的研究表明，端到端图像字幕系统依赖于匹配图像表示来生成字幕，并且对对象的频率，大小和位置进行编码是互补的，并且所有这些都在形成良好的图像表示中起作用。它还揭示了不同的对象类别以不同的方式对图像字幕做出贡献。
