...
离焦深度（DfD）和立体匹配是两个最被研究的被动深度感测方案。这些技术基本上是相辅相成的：DfD可以稳健地处理对于立体匹配有问题的重复纹理，而立体匹配对散焦模糊不敏感，并且可以处理较大的深度范围。在本文中，我们提出了一个统一的基于学习的技术来进行混合DfD和立体匹配。我们的输入是图像三联体：立体视图之一的立体像对和散焦图像。我们首先应用深度引导光场渲染来构建这种混合传感设置的综合训练数据集。接下来，我们采用沙漏网络架构分别从DfD和立体声进行深度推理。最后，我们利用两个独立网络之间的不同连接方法将它们集成到一个统一的解决方案中，以生成高保真三维视差图。对真实和合成数据的综合实验表明，我们新的基于学习的混合三维感知技术可以显着提高三维重建的准确性和鲁棒性。

##### URL
[https://arxiv.org/abs/1708.00583](https://arxiv.org/abs/1708.00583)

##### PDF
[https://arxiv.org/pdf/1708.00583](https://arxiv.org/pdf/1708.00583)
