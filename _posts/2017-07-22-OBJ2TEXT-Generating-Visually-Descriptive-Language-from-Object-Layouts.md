...
为图像生成字幕是最近受到相当大关注的一项任务。在这项工作中，我们主要关注抽象场景的字幕生成，或者对象布局，其中唯一提供的信息是一组对象及其位置。我们提出了OBJ2TEXT，一种序列到序列模型，它使用LSTM网络将一组对象及其位置编码为输入序列，并使用LSTM语言模型对这种表示进行解码。我们表明，尽管将对象布局编码为序列，我们的模型可以表示对象之间的空间关系，并生成全局一致且语义相关的描述。我们通过仅使用对象注释作为输入来测试我们的对象布局字幕任务的方法。我们另外显示，我们的模型，结合最先进的目标检测器，在标准MS-COCO字幕任务的测试基准中，将图像字幕模型从0.863提高到0.950（CIDEr得分）。
