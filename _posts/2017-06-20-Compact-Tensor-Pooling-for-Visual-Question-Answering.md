...
执行高水平的认知任务需要结合完全不同的特征地图。在视觉问答（VQA）中，图像描述符具有空间结构，而词汇输入固有地遵循时间序列。最近提出的多模式紧凑双线性池（MCB）通过计数草图近似形成在每个空间位置处的视觉和文本表示的外部产品。虽然这个过程在本地保存空间信息，但是对于活化张量的每个纤维独立地采用外积，因此不包括空间上下文。在这项工作中，我们引入了多维素描（{MD-sketch}），这是一种对草图进行张量计算的新颖延伸。使用这个新的公式，我们提出了多模式紧张张量池（MCT）来充分利用双线性汇聚操作中的全局空间环境。与MCB相反，我们的方法通过直接将来自视觉张量特征的MD草图与使用高阶FFT的文本向量特征进行卷积来保存空间上下文。此外，我们在问题的每个步骤中递增地应用MCT，并在选择最终答案之前用第二LSTM层积累多模式向量。
