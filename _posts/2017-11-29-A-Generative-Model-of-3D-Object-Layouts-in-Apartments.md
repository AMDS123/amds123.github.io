...
理解室内场景是计算机视觉中的一项重要任务。这个任务通常是模棱两可的，所以我们需要一个强大的先验，捕捉室内环境的规律性。这自然地通过三维空间布局和几何上的概率模型来表达，推理三维空间中的复杂布局，包括对象之间的高阶空间关系。在这项工作中，我们构建了这样一个模型，训练了超过25万个设计了170个对象的人性化房间。我们进行广泛的实验来展示我们的模型的质量。首先，通过广泛的用户研究，我们发现它生成的样本是合理的，包括将样本布局与实际情况进行人为比较。其次，我们展示了将对象之间的空间关系结合起来的价值，通过证明这增加了样本的合理性。第三，我们展示了我们的模型概括，而不是简单地记住它的训练集。最后，我们提供了很多我们模型学到的知识的例子，如支持关系，以及对象类之间的共同空间关系。
