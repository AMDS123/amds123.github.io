...
目前，低秩张量完成在恢复部分元素丢失的不完整视觉数据方面已经引起了人们的重视。通过将彩色图像或视频作为三维（3D）张量，以前的研究提出了张量核范数的几个定义。但是，它们有局限性，可能无法正确地逼近张量的真实等级。此外，他们没有明确使用低级属性进行优化。证明了最近提出的截断核范数（TNN）可以代替传统的核范数，作为对矩阵秩的更好的估计。因此，本文提出了一种称为张量截断核范数（T-TNN）的新方法，该方法提出了张量核范数的一个新的定义，并将截断核范数从矩阵范围扩展到张量情形。有益于TNN的低级别，我们的方法提高了张量完成的功效。我们利用先前提出的张量奇异值分解和乘法器交替方向法进行优化。对实际视频和图像进行的大量实验表明，我们的方法性能优于现有的方法。
