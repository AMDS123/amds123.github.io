...
我们提出了一个依赖于多区域深度卷积神经网络（CNN）的对象检测系统，其也编码语义分割感知特征。所得到的基于CNN的表示旨在捕获多样化的判别性外观因素，并展现对精确对象定位至关重要的定位灵敏度。我们利用我们的识别模块的上述属性，将其集成到一个迭代本地化机制上，这个机制在一个盒子提案的评分和CNN回归模型的深度之间交替进行。由于我们模块的高效使用，我们能够以非常高的定位精度检测物体。对于PASCAL VOC2007和PASCAL VOC2012的检测挑战，我们相应地实现了78.2％和73.9％的MAP，超出了任何其他已发表的工作的显着幅度。
