...
由于生成对抗网络（GAN），特别是Wasserstein GANs的巨大成功，无监督图像生成最近受到越来越多的关注。受到实值图像生成范例的启发，本文首先尝试提出了在实际应用中经常遇到的生成多值图像的问题。为了研究，我们专门开发了三个典型的流形值图像生成任务：色彩饱和度值（HSV）彩色图像生成，色度 - 亮度（CB）彩色图像生成和扩散张量（DT）图像生成。为了产生尽可能逼真的图像，我们将瓦塞尔斯坦GANs的最新技术推广到黎曼几何学的流形上下文中。对于所提出的多值图像生成问题，我们推荐三个基准数据集，即CIFAR-10 HSV / CB彩色图像，ImageNet HSV / CB彩色图像，UCL DT图像数据集。在三个数据集上，我们通过实验证明了所提出的具有流形意识的Wasserestein GAN可以生成高质量的流形值图像。
