...
视觉问答（VQA）具有挑战性，因为它需要同时理解图像的视觉内容和问题的文本内容。为了支持VQA任务，我们需要为以下三个问题找到好的解决方案：1）图像和问题的细粒度特征表示; 2）多模态特征融合，能够捕捉多模态特征之间复杂的相互作用; 3）自动答案预测能够考虑同一问题的多种不同答案之间的复杂相关性。对于细粒度的图像和问题表示，通过使用深度神经网络体系结构共同学习图像和问题的注意力，可以使我们有效地减少不相关的特征，获得“共同关注”机制更多的图像和问题表征的判别特征。对于多模态特征融合，开发了一种广义多模式分解高阶汇聚方法（MFH），通过充分利用它们之间的相关性，实现更有效的多模态特征融合，从而进一步提高VQA性能最先进的方法。对于答案预测，使用KL（Kullback-Leibler）散度作为损失函数，以更精确地表征具有相同或相似含义的多个不同答案之间的复杂相关性，这可以使我们获得更快的收敛速度并获得稍好答案预测的准确性。深度神经网络架构被设计为将所有这些上述模块集成到一个统一模型中以实现优越的VQA性能。通过9个模型的合奏，我们在大规模的VQA数据集上实现了最先进的性能，并赢得了2015年VQA挑战赛的亚军。
