...
最近的研究表明，广泛使用的深度神经网络（DNNs）容易受到精心设计的对抗性例子的影响。已经提出了许多先进的算法来通过利用$ \ mathcal {L} _p $距离来惩罚扰动来产生敌对的例子。研究人员已经探索了不同的防御方法来抵御这种对抗性攻击。尽管$ \ mathcal {L} _p $距离作为感知质量度量的有效性仍然是一个活跃的研究领域，但在本文中，我们将集中讨论不同类型的扰动，即空间变换，而不是操纵像素值直接和以前的作品一样。通过空间转换产生的扰动可能导致大量的数学距离测量，但是我们广泛的实验表明，这种空间变换的对抗性例子是现实的防御系统，在感觉上是现实的，更难以抵御。这可能为敌对的例子生成和相应的防御设计提供了一个新的方向。我们将基于空间变换的摄动视为不同的例子，并表明我们的技术可以产生平滑的图像变形的现实敌对的例子。最后，我们通过不同类型的对抗实例，将深度网络的注意力形象化，以更好地理解这些例子的解释。

##### URL
[https://arxiv.org/abs/1801.02612](https://arxiv.org/abs/1801.02612)

##### PDF
[https://arxiv.org/pdf/1801.02612](https://arxiv.org/pdf/1801.02612)
