...
有条件的生成对抗网络允许生成以某些外部信息为条件的样本。能够从条件GAN恢复潜在和条件向量可能在各种应用中具有潜在的价值，从用于娱乐目的的图像处理到用于安全目的的神经网络的诊断。在这项工作中，我们表明，有可能从生成的图像中恢复潜在和条件向量给定的条件生成敌对网络的发电机。由于深度神经网络通常具有多层次的非线性，这种恢复并不是微不足道的。此外，研究了这种恢复对真实自然图像的影响。我们发现在生成的和真实的图像之间的恢复性能之间存在差距，我们认为这是来自生成的数据分布和真实数据分布之间的差异。实验进行评估恢复的条件向量和重建的图像从这些恢复的载体定量和定性，显示有前途的结果。

##### URL
[http://arxiv.org/abs/1712.01833](http://arxiv.org/abs/1712.01833)

##### PDF
[http://arxiv.org/pdf/1712.01833](http://arxiv.org/pdf/1712.01833)
