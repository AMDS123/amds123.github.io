...
本文提出了在未来的SLAM问题中应用对象检测的最新方法。尽管提出了许多SLAM方法来为移动机器人即地面车辆创造适宜的自主性，但是在进入具有许多地标的巨大空间时仍然面临过度自信和大量计算。特别是，它们仅仅依靠像摄像机这样的有限的传感器而遭受不切实际的应用。所提出的方法要求无人驾驶的地面车辆没有大量数据库用于物体定义和高度预测参数，可以实时处理摄像机直线运动时的传入物体。线性（LC）滤波器尝试将检测，跟踪和学习应用到每个定义的专家，以获得更多的信息，用于判断场景而不会过度计算。在这个过滤器中，圈子专家让我们总结一下边界。每个专家之间的互动反馈学习创造的最小错误，在没有绘图的拥挤的场景中对抗压倒性的标志性标志。我们的专家基本上依赖于信任因素与几何定义的协方差来忽略，出现和比较检测到的地标。使用IMU传感器旁边的相机进行用于验证模型的实验以用于位置估计。
