...
最近，视觉问答（VQA）任务在人工智能中越来越受到重视。现有的VQA方法主要采用视觉注意机制将输入问题与相应的图像区域联系起来进行有效的问答。基于自由形式区域和基于检测的视觉注意机制的研究大都是基于自由形式的区域，而后者则是参与预定义的检测框区域。我们认为，两种关注机制能够提供补充信息，应该有效整合，以更好地解决VQA问题。在本文中，我们提出了一个新的VQA深度神经网络，它集成了两个注意机制。我们提出的框架通过多模式乘性特征嵌入方案有效地融合了自由形式图像区域，检测框和问题表示的特征，共同参与问题相关的自由形式图像区域和检测框，以更精确地回答问题。所提出的方法在两个公开可用的数据集COCO-QA和VQA上进行了广泛的评估，并且胜过了最新的方法。源代码可在此https网址获得
