...
目标检测是自主驾驶的关键任务。除了要求高精度以确保安全外，自主驾驶的物体检测还需要实时推理速度，以保证车辆控制的及时性，以及较小的模型尺寸和能源效率，以实现嵌入式系统部署。在这项工作中，我们提出SqueezeDet，一个完全卷积神经网络的目标检测，旨在同时满足所有上述约束。在我们的网络中，我们不仅使用卷积图层来提取特征图，而且还使用卷积图来计算边界框和类概率。我们模型的检测流水线只包含一个神经网络的正向通道，因此速度非常快。我们的模型是完全卷积的，导致模型尺寸小，能源效率更高。最后，我们的实验表明，我们的模型非常准确，在KITTI基准测试中达到了最新的精确度。
