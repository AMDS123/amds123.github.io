...
近年来，随着在线视频的大量增长，视频快速检索研究受到越来越多的关注。作为图像哈希技术的扩展，传统的视频哈希方法主要依赖于手工特征，将实值特征转化为二值哈希码。由于视频提供的视觉信息要比图像多得多，复杂，因此从视频中提取特征要比从图像中提取特征要困难得多。因此，需要表示视频的高级语义特征而不是低级别的手工方法。本文提出了深层卷积神经网络来提取高层次的语义特征，然后将二叉散列函数整合到该框架中，实现端到端的优化。特别是，我们的方法还结合了保留视频的相对相似性和差异性的三重丢失函数和分类损失函数作为优化目标。已经对两个公共数据集进行了实验，结果证明了我们提出的方法与其他最先进的视频检索方法相比的优越性。
