...
在基于CNN的目标检测方法中，当物体表现出明显的尺度变化，遮挡或截断时，区域提议成为瓶颈。另外，这些方法主要集中在二维物体检测上，不能估计物体的细节特性。在本文中，我们提出了子类别感知CNN的对象检测。我们引入一个新的区域提案网络，使用子类别信息来指导提案生成过程，并建立一个新的联合检测和子类别分类检测网络。通过使用与对象姿态相关的子类别，我们在常用基准的检测和姿态估计上实现了最先进的性能。
