...
当前的图像转换和重新着色算法试图在拍摄的图像中引入艺术效果，基于目标图像的用户输入或者预先设计的滤波器的选择。尽管这些操作旨在增强图像对观看者的影响，但是不包括通过指定情感信息来进行图像转换的选项。在本文中，我们提出了一种自动图像转换方法，可以根据用户的需要转换源图像，以便能够引起观看者的情绪影响。我们提出的新颖的图像情感传递算法不需要用户指定的目标图像。该算法利用深层卷积神经网络顶层提取的特征和用户指定的情感分布，从图像数据库中选取多个目标图像进行颜色变换，使得合成图像具有期望的情感影响。与以前的方法相比，我们的方法可以处理更多不同的照片。我们进行了详细的用户研究，展示了我们提出的方法的有效性。还提供了失败案例的讨论和推理，表明基于色彩转移的方法在使用情感分配方面的内在限制。

##### URL
[http://arxiv.org/abs/1707.08148](http://arxiv.org/abs/1707.08148)

##### PDF
[http://arxiv.org/pdf/1707.08148](http://arxiv.org/pdf/1707.08148)
