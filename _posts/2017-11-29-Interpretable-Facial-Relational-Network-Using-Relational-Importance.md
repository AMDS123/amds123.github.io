...
人脸分析是计算机视觉中的一项重要任务。根据认知心理学研究，面部动态可以为面部分析提供重要线索。特别是面部局部区域在面部表情中的运动与其他面部区域的运动有关。本文提出了一种利用面部局部动力学关系的深度学习方法从表情序列中估计面部特征。为了充分利用局部人脸动态关系，提出了一种面部局部动态特征编码网络和人脸关系网络。面部关系网络被设计为可解释的。关系重要性被自动编码，通过结合关系重要性的关系特征来估计面部特征。面部特征估计的面部动力学关系可以用关系重要性来解释。通过对比实验，验证了所提方法的有效性。实验结果表明，所提出的方法在性别和年龄估计方面优于现有的方法。
