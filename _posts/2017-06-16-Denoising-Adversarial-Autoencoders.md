...
date: 2017-06-16 16:07:58
categories: arXiv_CV
tags: arXiv_CV Adversarial Inference Classification
...
无监督学习越来越引起人们的兴趣，因为它释放了大量未标记数据的潜力，以学习有用的推理表示。可以通过学习从潜在表示空间重建未标记的输入数据来训练自动编码器（一种生成模型的形式）。如果学习从损坏的输入样本中恢复，则可以由自动编码器生成更健壮的表示。通过在训练期间引入规则化来形成编码数据在潜在空间中的分布，可以进一步改善表示。我们建议去噪对手自编码器结合去噪和正则化，利用对抗训练形成潜在空间的分布。我们介绍了一个新的分析，显示如何去噪可以纳入敌对自动编码器的训练和采样。进行实验以评估去噪对分类和样本合成表示学习的贡献。我们的研究结果表明，使用去噪标准进行训练的自编码器可以实现更高的分类性能，并且可以合成与未经腐败过程训练的样本相比，更符合输入数据的样本。

##### URL
[https://arxiv.org/abs/1703.01220](https://arxiv.org/abs/1703.01220)

##### PDF
[https://arxiv.org/pdf/1703.01220](https://arxiv.org/pdf/1703.01220)
