...
体素着色是从一组校准的2D图像重建三维表面模型的流行方法。然而，重建质量在很大程度上取决于阈值程序，允许作者为每个体素决定它是否是照片一致的。即便如此，由于其简单性和低计算成本，该方法被广泛使用。我们已经回到这种方法，以便提出一个完全自动化的阈值处理步骤的改进。实际上，使用滞后阈值来隐含地集成几何信息，该滞后阈值考虑了颜色体素的空间相干性。此外，通过将每个体素的模糊度隶属度定义为一致体素类，极大地减小了选择阈值的模糊性。而且，不需要预设阈值，因为根据体素被投射到的图像的数量自动且自适应地定义滞后阈值。初步结果非常有希望，并证明所提出的方法自动执行精确和平滑的体积场景重建。

##### URL
[https://arxiv.org/abs/1809.06070](https://arxiv.org/abs/1809.06070)

##### PDF
[https://arxiv.org/pdf/1809.06070](https://arxiv.org/pdf/1809.06070)
