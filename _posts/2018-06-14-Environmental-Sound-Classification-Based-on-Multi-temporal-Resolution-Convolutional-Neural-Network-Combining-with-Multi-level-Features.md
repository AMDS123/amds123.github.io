...
date: 2018-06-14 02:34:13
categories: arXiv_SD
tags: arXiv_SD CNN Classification
author: Boqing Zhu, Kele Xu, Dezhi Wang, Lilun Zhang, Bo Li, Yuxing Peng
mathjax: true
---
...
由于不同声级在不同时间尺度和层次级别上具有高度多样性的特点，提出了一种新的深度卷积神经网络（CNN）体系结构用于环境声音分类任务。该网络架构以原始波形作为输入，并且使用具有不同卷积滤波器大小和跨度的一组分离的并行CNN，以便学习具有多时间分辨率的特征表示。另一方面，所提出的体系结构还汇集了来自多级CNN层的分层特征，用于使用卷积层之间的直接连接进行分类，这超出了大多数先前研究所使用的典型单级CNN特征。这种网络架构还改善了信息流，避免了渐变问题。多级功能的组合极大地提高了分类性能。比较实验在两个数据集上进行：环境声音分类数据集（ESC-50）和DCASE 2017音频场景分类数据集。结果表明，所提出的方法通过采用多时间分辨率和多层次特征在分类任务中非常有效，并且优于以前仅考虑单层特征的方法。

##### URL
[http://arxiv.org/abs/1805.09752](http://arxiv.org/abs/1805.09752)

##### PDF
[http://arxiv.org/pdf/1805.09752](http://arxiv.org/pdf/1805.09752)
