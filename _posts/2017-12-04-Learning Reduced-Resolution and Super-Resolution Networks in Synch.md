...
最近的研究表明，深度卷积神经网络在图像超分辨率方面取得了优异的性能。然而，基于CNN的方法很大程度上依靠插值来恢复超分辨率结果。在本文中，我们提出了一种低分辨率和超分辨率的端到端网络（Reduced＆amp; Super-Resolution Network，RSRNet），没有任何便宜的插值。 1）降低分辨率的网络在不受监督的情况下进行训练，保留更有效的信息，并在LR图像中带来更好的视觉效果。 2）超分辨率网络学习密集连接的子像素残差，加速收敛，提高性能。对于图像超分辨率，我们评估提出的网络上的四个基准数据集，并设置了一个新的艺术状态。而且，RSRNet中降低分辨率的网络适用于生成照片般逼真的低分辨率图像，并通过使用现有的图像编解码器来提高图像压缩。
