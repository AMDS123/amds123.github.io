...
基于属性的人员重新识别（Re-ID）旨在使用属性查询搜索大型图像数据库中的人员。尽管有很大的实际意义，语义属性和视觉图像之间的巨大差距使其成为一个非常具有挑战性的任务。现有的研究主要集中在查询属性与人类图像属性评分之间的匹配问题，属性预测不完善，可辨识度低。在这项工作中，我们提出制定基于属性的人Re-ID作为联合空间学习问题。为了缓解不同模式之间巨大的异构差异所带来的负面影响，我们采用新颖的对手培训策略，为两种模式生成同质特征，提供特征层面不同模式之间的分布对齐，并保持模式之间的语义一致性。我们的实验验证了我们的模型的有效性，并且在性能上比现有技术模型有了很大的提高。
