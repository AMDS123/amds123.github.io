...
用于识别阿尔茨海默病（AD）的数据集通常相对较少，这限制了他们培养可推广的模型的能力。在这里，我们用两个规范数据集，Wisconsin纵向研究和Talk2Me中的每一个来扩充这样一个数据集DementiaBank，每个数据集采用基于语音的图片描述评估。通过使用ADASYN进行少数类别过采样，我们超越了痴呆银行AD和AD患者二元分类的最新结果。这项工作突出了将稀疏和难以获取的患者数据与相对较大且易于访问的规范数据集相结合的有效性。
