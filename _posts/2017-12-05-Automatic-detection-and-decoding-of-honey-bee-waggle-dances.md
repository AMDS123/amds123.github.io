...
摇摆舞是动物交流最流行的例子之一。 Forager蜜蜂通过一个复杂的电机显示屏指挥他们的巢穴有利可图的资源。本质上，舞蹈将极坐标编码到场中的资源。失业的牧羊人跟随舞者的动作，然后搜索在现场广告斑点。在过去的几十年中，生物学家采用了不同的技术来衡量摆动舞蹈的关键特征，并解读它传达的信息。早期的技术包括使用量角器和秒表直接从观察蜂房测量舞蹈的方向和持续时间。最近的方法在屏幕上使用数字录像和手动测量。但是，手动方法非常耗时。因此，大多数研究在短时间内只考虑少量的动物。我们开发了一个能够实时自动检测，解码和映射通信舞蹈的系统。在本文中，我们描述了我们的记录设置，为跳舞检测和解码执行的图像处理步骤以及将舞蹈映射到现场的算法。所提出的系统以90.07％的检测精度执行。解码后的摆动方向的平均误差为-2.92 {\ deg}（$ \ pm $ 7.37 {\ deg}），在人为错误的范围内。为了评估和体现系统的性能，一群蜜蜂接受了人工喂食器的训练，所有群体的舞蹈都被自动检测，解码和映射。这里介绍的系统是第一个公开可用的系统，包括源代码和硬件规格。我们希望这将促进蜜蜂摇摆舞的定量分析。
