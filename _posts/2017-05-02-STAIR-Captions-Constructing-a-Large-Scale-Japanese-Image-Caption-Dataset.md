...
近年来，图像描述（字幕）的自动生成，即图像字幕，引起了很大的关注。在本文中，我们特别考虑为图像生成日文字幕。由于大多数可用的字幕数据集都是为英语而建的，因此日语的数据集很少。为了解决这个问题，我们基于MS-COCO的图像构建了一个大型的日本图像标题数据集，称为STAIR字幕。 STAIR字幕包含820,310个日文字幕，用于164,062张图片。在实验中，我们发现使用STAIR Caption训练的神经网络与生成英文字幕后使用英文 - 日文机器翻译生成的相比，可以生成更自然，更好的日文字幕。
