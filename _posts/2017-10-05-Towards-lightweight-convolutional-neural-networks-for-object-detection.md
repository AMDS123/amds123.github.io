...
我们提出了具有较大空间尺寸特征图的模型，并在物体检测任务上进行评估。为了为我们的模型选择最好的特征提取网络，我们比较了几个流行的轻量级网络。之后，我们进行一系列的频道缩减算法实验，以加速执行。我们的车辆检测模型准确，快速，因此适合嵌入式视觉应用。只有1.5 GFLOP我们最好的模型给出了具有挑战性的DETRAC数据集的验证子集93.39 AP。我们最小的型号是第一个在CPU上达到实时推断速度，合理精度降至91.43AP。
