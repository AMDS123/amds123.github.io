...
高维数据的生成建模是机器学习中的关键问题。成功的方法包括潜变量模型和自回归模型。这些方法的互补优势，分别模拟全球和本地图像统计，建议结合两种模式的优势混合模型。我们的贡献是使用辅助损失函数来训练这样的混合模型，该函数控制哪些信息被潜变量捕获，以及遗留给自回归解码器的是什么。相比之下，此类混合模型的工作需要限制自回归解码器的能力，以防止忽略潜变量的退化模型，而只依赖于自回归建模。我们的方法导致具有有意义的潜在变量表示的模型，并依靠强大的自回归解码器来模拟图像细节。我们的模型可以生成具有说服力的质量样品，并获得最新的定量结果。
