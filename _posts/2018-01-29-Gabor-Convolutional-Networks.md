...
传统滤波器（例如Gabor滤波器）的设计具有可操纵的特性，赋予特征处理空间变换的能力。然而，在流行的深度卷积神经网络（DCNN）中，这样的优异特性尚未得到很好的研究。在本文中，我们提出了一种新的深度模型，称为Gabor卷积网络（GCNs或Gabor CNNs），其将Gabor滤波器并入到DCNN中，以增强深度学习特征对方向和尺度变化的抵抗力。通过仅基于Gabor滤波器（即卷积运算符）操作DCNN的基本元素，GCN可以容易地实现并且与任何流行的深度学习体系结构兼容。实验结果证明了我们的算法在识别物体的超强能力，其中规模和旋转的变化频繁发生。所提出的GCN具有少得多的可学习的网络参数，因此更容易用端到端的流水线进行训练。

##### URL
[http://arxiv.org/abs/1705.01450](http://arxiv.org/abs/1705.01450)

##### PDF
[http://arxiv.org/pdf/1705.01450](http://arxiv.org/pdf/1705.01450)
