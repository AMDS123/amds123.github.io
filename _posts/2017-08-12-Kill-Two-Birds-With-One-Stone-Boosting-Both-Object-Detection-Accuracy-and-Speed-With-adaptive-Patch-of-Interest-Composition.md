...
date: 2017-08-12 16:52:43
categories: arXiv_CV
tags: arXiv_CV Video_Caption Object_Detection Detection
...
在视频理解和分析中，对象检测是一项重要且具有挑战性的任务，其中一个主要挑战在于两个相互矛盾的因素之间的适当平衡：检测准确度和检测速度。在本文中，我们提出了一种新的自适应补丁组合方法来提高对象检测的准确性和速度。所提出的方法首先在视频帧中提取可能包括感兴趣对象的片段。然后，引入自适应合成过程，将提取的拼块组合成用于物体检测的最佳子帧数。通过这个过程，我们能够在对象检测过程中保持原始帧的分辨率（以保证精度），同时尽量减少检测输入的数量（以提高速度）。各种数据集上的实验结果证明了所提出方法的有效性。
