...
作为一般规则，数据分析现在对公司是强制性的。扫描文档分析带来了纸张损坏和扫描质量带来的额外挑战。在工业环境中，这项工作的重点是自动了解销售收据，从而能够获得基本准确的消费统计数据。鉴于通过智能手机获取的图像，所提出的工作主要集中在完整的工具链的第一步，旨在以最高的可信度提供店面品牌，采购产品和相关价格等重要信息。为了获得这个高置信水平，即使扫描没有得到完美的控制，我们一方面提出一个使用深度卷积神经网络（DCNNs）的双重检查处理工具链，另一方面提供更经典的图像和文本处理。工作涉及到这个双重核对处理和DCNN的联合使用，用于不同的应用和文本分析。
