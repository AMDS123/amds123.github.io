...
在最先进的神经机器翻译中，在解码期间使用注意机制来增强翻译。在每一步，解码器都使用这种机制来聚焦源句子的不同部分，以在输出其目标词之前收集最有用的信息。最近，关注机制的有效性也在多模式任务中得到了探索，在这些任务中，句子部分和图像区域都成为可能。汇集两种模式的方法通常包括基于元素的产品，总和或级联。在本文中，我们评估更先进的多模式紧凑双线性池化方法，它将两个向量的外积结合起来，将两种模式的注意特征结合起来。之前已经对视觉问题回答进行了调查。我们试用这种方法进行多模式图像标题翻译，并且与基本组合方法相比显示出改进。
