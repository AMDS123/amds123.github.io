...
本文提出了一种新的生成概率建模在逆强化学习方法，称为主题相关深度老化路径（SDAP），模型的面部结构和纵向面部老化过程的给定主题。所提出的SDAP使用基于卷积神经网络的深度特征提取的易处理对数似然目标函数进行优化。此外，SDAP不是针对所有输入人脸和主题使用固定的老化发展路径，而是针对优化奖励老化公式的每个主题提供最合适的老化发展路径。与以前只能将一个图像作为输入的方法不同，SDAP允许多个图像作为输入，即在相同或不同年龄的对象的所有信息，以产生对象的最佳老化路径。最后，SDAP允许在没有复杂的预处理步骤的情况下有效地合成在野外老化的面部。提出的方法在面部老化综合和跨年龄面部验证这两个任务上都进行了实验。实验结果始终如一地显示了在众多面部老化数据库（即FG-NET，MORPH，野生AginG面部（AGFW）和跨年龄名人数据库（CACD））上使用SDAP的最新性能。该方法还执行大规模的Megaface挑战1来展示所提出的解决方案的优点。
