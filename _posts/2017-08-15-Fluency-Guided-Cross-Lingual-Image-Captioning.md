...
迄今为止，图像字幕主要以英文进行了探索，因为大多数可用的数据集都是用这种语言编写的。然而，图像字幕的应用不应该受到语言的限制。对于跨语言环境中的图像字幕只进行了很少的研究。与这些手动构建目标语言数据集的作品不同，我们的目标是从机器翻译句子中完全学习一个跨语言字幕模型。为了克服翻译句子的流畅性，本文提出了一个流畅性的学习框架。该框架包括模块自动估计句子的流畅性和另一模块以利用所估计的分数流畅有效培养的图像字幕模式为目标语言。对两个双语（英汉）数据集的实验显示，我们的方法提高了中文生成的字幕的流畅性和相关性，但是没有使用任何来自目标语言的手写句子。
