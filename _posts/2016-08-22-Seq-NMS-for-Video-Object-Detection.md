...
视频对象检测是具有挑战性的，因为在一帧中容易检测到的对象可能难以在同一片段内的另一帧中检测到。近来，在单个图像中进行对象检测已经取得了重大进展。这些方法通常包含三个阶段：（i）对象建议生成（ii）对象分类和（iii）后处理。我们建议修改后期处理阶段，使用来自附近帧的高分对象检测来提高同一片段内较弱检测的分数。我们表明，我们的方法获得优越的结果，以最先进的单个图像对象检测技术。我们的方法在2015年ImageNet大规模视觉识别挑战（ILSVRC2015）的视频对象检测（VID）任务中排名第三。
