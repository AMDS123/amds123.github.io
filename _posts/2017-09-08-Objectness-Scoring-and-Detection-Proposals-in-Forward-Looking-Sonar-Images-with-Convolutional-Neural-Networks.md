...
前视声纳可以捕捉水下场景的高分辨率图像，但是它们的解释是复杂的。这种图像中的通用对象检测尚未解决，特别是在小型和未知对象的情况下。相比之下，检测建议算法已经在真实世界的彩色图像中产生了性能最好的物体检测器。在这项工作中，我们开发了一个卷积神经网络，可以可靠地评价前视声纳图像中的图像窗口的对象性，并通过阈值对象来产生检测建议。在我们的海洋垃圾对象数据集中，我们获得了94％的召回率，每个图像产生约60个提案。我们的方法最大的优点是它可以推广到以前看不见的对象。我们通过检测链环节，墙壁和扳手来展示这些，而没有在这些物体上进行过训练。我们坚信我们的方法可以用于与类别无关的对象检测，并具有许多真实世界的应用，如链接跟踪和地雷检测。
