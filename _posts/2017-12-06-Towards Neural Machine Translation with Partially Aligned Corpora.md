...
而神经机器翻译（NMT）已经成为新的范例，参数优化需要大量的并行数据，这在许多领域和语言对中是很少见的。在本文中，我们提出了一个新的翻译方案，其中只存在单语言语料库和短语对。我们提出了一种新的翻译方法，即从短语对和单语语料库中得到部分对齐的句对。为了充分利用部分对齐的语料库，我们对传统的NMT训练方法进行了两方面的调整。一方面，针对对齐和不对齐的目标词设计不同的生成策略。另一方面，设计不同的目标函数来模拟部分对齐的部分。实验证明，在这样的翻译场景下，我们的方法可以取得比较好的效果，微小的双字可以在很大程度上提高翻译质量。
