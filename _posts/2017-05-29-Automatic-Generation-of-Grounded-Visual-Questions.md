...
在本文中，我们提出了第一个模型，能够为单个图像生成不同类型的视觉基础问题。视觉问题的产生是一个新兴的主题，其目的是以视觉输入为基础，用自然语言提出问题。据我们所知，它缺乏自动的方法来产生相同的视觉输入各种类型的有意义的问题。为了避免这个问题，我们提出了一个模型，可以自动生成不同类型的视觉基础问题。我们的模型同时输入图像和密集字幕模型生成的字幕，对最可能的问题类型进行采样，并在后续中产生问题。两个实际数据集上的实验结果表明，我们的模型在正确性和多样性方面优于最强的基线。
