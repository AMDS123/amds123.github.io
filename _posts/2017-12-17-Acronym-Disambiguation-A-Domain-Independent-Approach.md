...
首字母缩写词无处不在。他们通常表达重复和众所周知的信息。但缩写词也可能是不明确的，因为对于同一首字母缩写词可以有多个扩展。在本文中，我们提出了一个通用的首字母缩略词消歧系统，可以根据给定的上下文信息对任何首字母缩略词进行处理。我们提出了从Wikipedia和AcronymsFinder.com检索所有可能的首字母缩写词的方法。我们建议使用这些扩展来收集使用这些缩略词的所有可能的上下文，然后使用称为Doc2Vec的段落嵌入技术对它们进行评分。这种方法共同导致在给定首字母缩略词选择正确扩展时，在我们从维基百科获取的具有707个不同首字母缩略词和14,876个消歧的数据集上达到了90.9％的准确度。
