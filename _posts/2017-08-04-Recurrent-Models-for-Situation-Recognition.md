...
这项工作提出了递归神经网络（RNN）模型来预测结构化的“形象情境” - 动作和名词实体履行与行动有关的语义角色。与先前依赖条件随机场（CRF）的工作相反，我们使用专门的动作预测网络，然后是RNN进行名词预测。我们的系统在具有挑战性的最新imSitu数据集上获得了最新的准确性，击败了基于CRF的模型，包括用额外数据进行训练的模型。此外，我们表明，从情景预测学习的专业特征可以转移到图像字幕的任务，以更准确地描述人与物体的相互作用。
