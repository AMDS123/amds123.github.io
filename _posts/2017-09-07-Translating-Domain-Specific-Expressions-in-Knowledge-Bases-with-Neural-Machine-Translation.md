...
categories: arXiv_CL
tags: arXiv_CL Knowledge_Graph Knowledge Segmentation NMT
...
我们在本文中提出的工作主要集中在语义结构化资源（如本体或知识图）中表示的领域特定表达式的翻译。为了使知识可以超越语言边界，这些资源需要翻译成不同的语言。翻译本体所代表的标签或术语表达的挑战在于高度特定的词汇和缺乏上下文信息，这可以指导机器翻译系统将歧义词翻译成目标领域。由于面临的挑战，我们使用统计学和神经机器翻译方法来训练和翻译中间和金融领域的术语表达。我们评估翻译质量的领域特定的表达与翻译系统在一般数据集和实验域适应与术语表达式的训练。此外，我们还进行了将外部知识注入翻译系统的实验。通过这些实验，我们观察到NMT方法在领域适应和术语注入方面的明显优势。尽管如此，通过特定的和独特的术语表达，NMT内的子词分割并没有超过基于单​​词的神经翻译模型。
