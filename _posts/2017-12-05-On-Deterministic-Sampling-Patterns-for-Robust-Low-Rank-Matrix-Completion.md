...
在这封信中，我们研究了确定性抽样模式，用于完成低秩矩阵，当它被稀疏噪声破坏时，也称为鲁棒矩阵完成。基于格拉斯曼流形上的几何分析，我们扩展了无噪声下确定性抽样模式的最新结果。一个特殊的情况，其中每列有一定数量的嘈杂条目被认为是我们的概率分析非常有效地执行。此外，假设没有给出原始矩阵的秩，我们通过验证一些条件来提供分析以确定有效完成的秩是否确实是用稀疏噪声破坏的数据的实际秩。
