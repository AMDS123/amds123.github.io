...
众所周知，上下文和多尺度表示对准确的视觉识别是重要的。在本文中，我们提出了内外网（ION），一种利用感兴趣区域内外的信息的对象检测器。利用空间递归神经网络整合感兴趣区域之外的情境信息。在内部，我们使用跳跃池来提取多个尺度和抽象层次上的信息。通过广泛的实验，我们评估设计空间，并为读者提供一个关于交易的诀窍是重要的概述。 ION将PASCAL VOC 2012对象检测技术的最新水平从73.9％提高到76.4％mAP。在新的和更具挑战性的MS COCO数据集上，我们改进了最先进的从19.7％到33.1％的mAP。 2015年MS COCO检测挑战赛中，我们的ION模型获得了最佳学生入围名单，并获得第三名。正如直觉所表明的，我们的检测结果提供了强有力的证据，表明上下文和多尺度表示改善了小物体检测。
