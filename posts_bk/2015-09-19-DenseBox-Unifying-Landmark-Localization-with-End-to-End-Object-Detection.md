...
单个完全卷积神经网络（FCN）如何执行物体检测？我们引入了DenseBox，一个统一的端到端FCN框架，通过图像的所有位置和尺度直接预测边界框和对象类的置信度。我们的贡献是双重的。首先，我们展示了一个FCN，如果设计和优化仔细，可以非常准确和有效地检测多个不同的对象。其次，我们证明在多任务学习中与地标本地化结合时，DenseBox进一步提高了物体检测的准确性。我们在包括MALF人脸检测和KITTI汽车检测在内的公共基准数据集上展示了实验结果，表明我们的DenseBox是用于检测诸如人脸和汽车等具有挑战性的物体的最先进的系统。
